...
import combat_idle_down_weapon_variants_builder_v07 as previous_builder
...
ONE_HAND_LOW_V08_OBJECT_NAMES = (
    "combat_onehand_low_v08_blade",
    "combat_onehand_low_v08_highlight",
    "combat_onehand_low_v08_tip",
    "combat_onehand_low_v08_guard",
    "combat_onehand_low_v08_grip",
    "combat_onehand_low_v08_pommel",
)
ONE_HAND_READY_V08_OBJECT_NAMES = (
    "combat_onehand_ready_v08_blade",
    "combat_onehand_ready_v08_highlight",
    "combat_onehand_ready_v08_tip",
    "combat_onehand_ready_v08_guard",
    "combat_onehand_ready_v08_grip",
    "combat_onehand_ready_v08_pommel",
)
...
def _register_one_hand_part(
    ...
    *,
    ...
    registered["weapon_id"] = "sword_01_onehand_side_back_v08"
    registered["weapon_part"] = weapon_part
    registered["weapon_state"] = "drawn"
    registered["weapon_variant_id"] = variant_id
    registered["combat_idle_weapon_variants_revision"] = "v08"
    registered["one_hand_trajectory"] = "lateral_and_partly_behind_hero"
    registered.hide_render = True
    registered.hide_viewport = True
    return registered


def _build_one_hand_v08(
    context: factory.BuildContext,
    *,
    variant_id: str,
    prefix: str,
...
def create_weapon_stance_actions_v08(context: factory.BuildContext) -> None:
    ...
    for variant in profile.variants[:2]:
        ...
        if factory.bpy.data.actions.get(action_name) is not None:
            raise RuntimeError(f"combat idle v08 action already exists: {action_name}")
        action = action_builder._create_action(context, variant)
        action["profile_revision"] = "v08"
        action["one_hand_blade_lateral_and_partly_behind"] = True
        action["body_pose_source_revision"] = "v06"
        action["two_hand_source_revision"] = "v06"
        action["supersedes_occluded_one_hand_revision"] = "v07"
    ...
    low_parts = _build_one_hand_v08(
        context,
        variant_id=one_hand_low.variant_id,
        prefix="combat_onehand_low_v08",
    )
    ...
    factory.bpy.context.scene.frame_set(1)
    factory.bpy.context.view_layer.update()
    ready_parts = _build_one_hand_v08(
        context,
        variant_id=one_hand_ready.variant_id,
        prefix="combat_onehand_ready_v08",
    )
    ...
    scene["combat_idle_weapon_variants_revision"] = "v08"
    scene["combat_idle_weapon_variant_count"] = len(profile.variants)
    scene["combat_idle_one_hand_low_v08_object_count"] = len(low_parts)
    scene["combat_idle_one_hand_ready_v08_object_count"] = len(ready_parts)
    scene["combat_idle_one_hand_separate_pose_fitted_modules"] = True
    scene["combat_idle_one_hand_lateral_and_partly_behind"] = True
    scene["combat_idle_one_hand_v07_rejected_for_occlusion"] = True
