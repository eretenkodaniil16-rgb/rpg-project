...
from combat_idle_down_variants_builder_v04 import (
    create_combat_idle_down_variant_actions_v04,
)
...
PROFILE_PATH = SCRIPT_DIR / "combat_idle_down_variants_profile_v04.py"
BUILDER_PATH = SCRIPT_DIR / "combat_idle_down_variants_builder_v04.py"
COMPARISON_SHEET_NAME = "combat_idle_down_variants_v04.png"


def render_pilot_combat_idle_down_variants_v04(
    ...
    profile = load_combat_idle_down_variants_profile_v04(config.character_id)
    ...
    try:
        ...
        for variant in profile.variants:
            ...
            if action is None:
                raise RuntimeError(
                    f"combat_idle_down variants v04 cannot find action {action_name}"
                )
...
def _candidate_paths(
    ...
    animation_ids = (
        "combat_idle",
        "combat_idle_wide_low_v04",
        "combat_idle_wide_ready_v04",
        "combat_idle_wide_high_v04",
    )
    paths: list[Path] = []
    for animation_id in animation_ids:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"combat_idle_down variants v04 requires one {animation_id} frame, "
                f"got {len(matches)}"
            )
...
def _write_variant_comparison_sheet(
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_combat_idle_down_variants_v04",
        width=width,
        height=height,
        alpha=True,
        float_buffer=False,
...
def _write_contact_sheet_combat_idle_down_variants_v04(
    config: object,
    artifacts: list[factory.FrameArtifact],
    output_path: Path,
) -> Path:
...
def _write_run_manifest_combat_idle_down_variants_v04(
    context: factory.BuildContext,
    run_dir: Path,
    run_id: str,
    blend_path: Path,
    ...
    if not comparison_path.is_file():
        raise RuntimeError("combat_idle_down variants v04 comparison sheet is missing")
    ...
    for variant in profile.variants:
        ...
        if action is None or action.get("profile_revision") != "v04":
            raise RuntimeError(
                f"combat_idle_down variants v04 action drifted: {variant.variant_id}"
            )
        matches = [
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"combat_idle_down variants v04 artifact drifted: {variant.variant_id}"
            )
        artifact = matches[0]
    ...
    payload["combat_idle_down_variants_v04"] = {
        ...
        "rejected_revisions": (
            {
                "revision": "v02",
                "reason": "arms_and_blade_not_lateral_enough_for_requested_stance",
            },
            {
                "revision": "v03",
                "reason": "closed_guard_converged_toward_torso_center",
            },
        ),
        ...
        "locked_contract": {
            ...
            "both_arms_move_away_from_torso_center": True,
            "blade_remains_on_physical_right_side": True,
            "scabbard_remains_physical_left": True,
            "mirroring_used": False,
            "negative_scale_used": False,
        },
        "status": "technical_wide_side_variant_set_requires_manual_selection",
    }
    payload.setdefault("animation_contract", {})
    payload["animation_contract"].update(
        {
            "combat_idle_down_active_stage": "variants_v04",
            "combat_idle_down_variant_count": len(profile.variants),
            "combat_idle_down_manual_selection_required": True,
            "combat_idle_down_wide_side_guard": True,
        }
    )
    payload.setdefault("appearance_candidate", {})
    payload["appearance_candidate"]["status"] = (
        "artist_approved_appearance_and_walk_set_with_wide_side_"
        "combat_idle_down_variants_v04_pending_selection"
    )
...
def main() -> int:
    ...
    base_adapter._write_contact_sheet_combat_idle_down_v01 = (
        _write_contact_sheet_combat_idle_down_variants_v04
    )
    base_adapter._write_run_manifest_combat_idle_down_v01 = (
        _write_run_manifest_combat_idle_down_variants_v04
    )
