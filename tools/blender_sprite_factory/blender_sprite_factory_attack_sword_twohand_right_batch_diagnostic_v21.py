...
DIAGNOSTIC_SCENE_KEY = "attack_sword_twohand_right_batch_diagnostic_v21"
CONTACT_SHEET_NAME = "attack_sword_01_twohand_right_batch_diagnostic_v21.png"
...
def _projection_target_direction(
    ...
    *,
    ...
    if source_projection <= 1.0e-6:
        raise RuntimeError(
            "two-hand right batch diagnostic source projection is degenerate"
        )
    ...
    *,
    ...
    try:
                    ...
                    try:
                        ...
                        artifact, _ = export_adapter._render_candidate(
                            ...
                            output_name=(
                                ...
                                f"twohand_right_batch_diagnostic_v21_"
                        ...
                        diagnostics.append(diagnostic)
                        print(
                            "ATTACK_SWORD_TWOHAND_RIGHT_BATCH_"
        ...
        if (
            ...
            raise RuntimeError(
                "two-hand right batch diagnostic found no safe candidate for "
        ...
        print(
            "ATTACK_SWORD_TWOHAND_RIGHT_BATCH_DIAGNOSTIC_V21_SELECTED="
            f"frame:{target_frame};"
            f"source:{source_frame};"
            f"source_kind:{source_kind};"
            f"blend:{float(selected['arm_blend']):.2f};"
            f"projection:{float(selected['applied_projection']):.6f};"
...
def _render_diagnostic(
    ...
    if action is None:
        raise RuntimeError(
            f"two-hand right batch diagnostic action is missing: {TARGET_ACTION_ID}"
        )
    ...
    try:
        ...
        payload = {
            "revision": TWOHAND_RIGHT_BATCH_DIAGNOSTIC_REVISION,
...
def _write_seven_frame_sheet(
    ...
    if [item.frame_number for item in ordered] != list(TARGET_FRAMES):
        raise RuntimeError(
            "two-hand right batch diagnostic expected frames f02-f08"
        )
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_attack_sword_twohand_right_batch_diagnostic_v21",
        width=width,
        height=height,
        alpha=True,
        float_buffer=False,
