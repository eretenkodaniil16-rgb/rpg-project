...
from combat_idle_down_weapon_variants_builder_v05 import (
    ...
    create_weapon_stance_actions_v05,
)
...
PROFILE_PATH = SCRIPT_DIR / "combat_idle_down_weapon_variants_profile_v05.py"
BUILDER_PATH = SCRIPT_DIR / "combat_idle_down_weapon_variants_builder_v05.py"
COMPARISON_SHEET_NAME = "combat_idle_down_weapon_variants_v05.png"
...
def _require_objects(names: tuple[str, ...], label: str) -> None:
    ...
    if missing:
        raise RuntimeError(f"combat idle v05 is missing {label} objects: {missing}")


def _set_weapon_mode(mode: str | None) -> None:
    _require_objects(COMBAT_WEAPON_OBJECT_NAMES, "baseline sword")
    _require_objects(ONE_HAND_LONG_OBJECT_NAMES, "one-hand long sword")
    _require_objects(TWO_HAND_LONG_OBJECT_NAMES, "two-hand long sword")
    _require_objects(SHEATHED_HILT_OBJECT_NAMES, "sheathed hilt")

    groups = {
        "baseline": COMBAT_WEAPON_OBJECT_NAMES,
        "one_handed": ONE_HAND_LONG_OBJECT_NAMES,
        "two_handed": TWO_HAND_LONG_OBJECT_NAMES,
    }
    for group_mode, names in groups.items():
        enabled = mode == group_mode
        for name in names:
            obj = factory.bpy.data.objects[name]
            obj.hide_render = not enabled
            obj.hide_viewport = not enabled
    drawn = mode is not None
...
def render_weapon_stance_variants_v05(
    ...
    profile = load_weapon_stance_profile_v05(config.character_id)
    ...
    try:
        for variant in profile.variants:
            ...
            if action is None:
                raise RuntimeError(
                    f"combat idle v05 cannot find action {variant.animation_id}"
                )
            ...
            _set_weapon_mode(variant.grip_mode)
...
def _comparison_paths(
    ...
    animation_ids = (
        "combat_idle_onehand_low_v05",
        "combat_idle_onehand_ready_v05",
        "combat_idle_twohand_center_low_v05",
        "combat_idle_twohand_center_high_v05",
    )
    paths: list[Path] = []
    for animation_id in animation_ids:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"combat idle v05 requires one {animation_id} frame, got {len(matches)}"
            )
...
def _write_variant_sheet(
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_combat_idle_down_weapon_variants_v05",
        width=width,
        height=height,
        alpha=True,
        float_buffer=False,
...
def _write_manifest_v05(
    ...
    if not comparison_path.is_file():
        raise RuntimeError("combat idle v05 comparison sheet is missing")
    ...
    for variant in profile.variants:
        ...
        if len(matches) != 1:
            raise RuntimeError(f"combat idle v05 artifact drifted: {variant.variant_id}")
        rendered_variants.append(_variant_payload(variant, matches[0]))

    payload["combat_idle_down_weapon_variants_v05"] = {
        ...
        "locked_contract": {
            ...
            "two_hand_blade_centered_tip_up": True,
            "separate_weapon_modules": True,
    ...
    payload["animation_contract"].update(
        {
            "combat_idle_down_active_stage": "weapon_variants_v05",
...
def main() -> int:
    base_adapter.create_combat_idle_down_actions_v01 = create_weapon_stance_actions_v05
    base_adapter.render_pilot_combat_idle_down_v01 = render_weapon_stance_variants_v05
    base_adapter._write_contact_sheet_combat_idle_down_v01 = _write_contact_sheet_v05
    base_adapter._write_run_manifest_combat_idle_down_v01 = _write_manifest_v05
