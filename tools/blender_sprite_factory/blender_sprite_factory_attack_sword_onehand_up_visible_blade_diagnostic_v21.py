...
def _visible_blade_head_clearance(objects: tuple[object, ...]) -> float:
    by_name = {str(obj.name): obj for obj in objects}
    missing = [name for name in BLADE_CLEARANCE_OBJECT_NAMES if name not in by_name]
    if missing:
        raise RuntimeError(
            "one-hand up visible blade diagnostic is missing collision objects: "
            + ", ".join(missing)
        )
    unexpected = [
        name
        for name in HILT_OCCLUSION_OBJECT_NAMES
        if name not in by_name
    ]
    if unexpected:
        raise RuntimeError(
            "one-hand up visible blade diagnostic is missing hilt objects: "
            + ", ".join(unexpected)
        )
    blade_objects = tuple(by_name[name] for name in BLADE_CLEARANCE_OBJECT_NAMES)
...
def _write_manifest_pass21(
    ...
    payload["attack_sword_directional_cycle_v21_pass21"] = {
        ...
        "collision_objects": list(BLADE_CLEARANCE_OBJECT_NAMES),
        "hilt_occlusion_objects": list(HILT_OCCLUSION_OBJECT_NAMES),
