...
import blender_sprite_factory_combat_idle_down_weapon_variants_v07 as previous_adapter
...
from combat_idle_down_weapon_variants_builder_v08 import (
    ...
    create_weapon_stance_actions_v08,
)
...
PROFILE_PATH = SCRIPT_DIR / "combat_idle_down_weapon_variants_profile_v08.py"
BUILDER_PATH = SCRIPT_DIR / "combat_idle_down_weapon_variants_builder_v08.py"
COMPARISON_SHEET_NAME = "combat_idle_down_weapon_variants_v08.png"
...
def _require_objects(names: tuple[str, ...], label: str) -> None:
    ...
    if missing:
        raise RuntimeError(f"combat idle v08 is missing {label} objects: {missing}")


def _set_weapon_variant_v08(variant_id: str | None) -> None:
    groups = (
    ...
    selected = {
        "onehand_low": ONE_HAND_LOW_V08_OBJECT_NAMES,
        "onehand_ready": ONE_HAND_READY_V08_OBJECT_NAMES,
        "twohand_center_low": TWO_HAND_LOW_V06_OBJECT_NAMES,
        "twohand_center_high": TWO_HAND_HIGH_V06_OBJECT_NAMES,
        "baseline": COMBAT_WEAPON_OBJECT_NAMES,
    }.get(variant_id, ())
...
def render_weapon_stance_variants_v08(
    ...
    profile = load_weapon_stance_profile_v08(config.character_id)
    calibration = previous_adapter.previous_adapter.calibration_adapter._calibrate_idle_down(
        context,
        run_dir,
    )
    raw_dir = run_dir / "raw"
    ...
    try:
        for variant in profile.variants:
            ...
            expected_revision = "v08" if variant.grip_mode == "one_handed" else "v06"
            if action is None or action.get("profile_revision") != expected_revision:
                raise RuntimeError(
                    f"combat idle v08 cannot find {expected_revision} action "
            ...
            _set_weapon_variant_v08(variant.variant_id)
...
def _comparison_paths(
    ...
    for animation_id in animation_ids:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"combat idle v08 requires one {animation_id} frame, got {len(matches)}"
            )
...
def _write_variant_sheet(
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_combat_idle_down_weapon_variants_v08",
        width=width,
        height=height,
        alpha=True,
        float_buffer=False,
...
def _write_manifest_v08(
    ...
    profile = load_weapon_stance_profile_v08(context.config.character_id)
    comparison_path = run_dir / COMPARISON_SHEET_NAME
    if not comparison_path.is_file():
        raise RuntimeError("combat idle v08 comparison sheet is missing")
    ...
    for variant in profile.variants:
        ...
        if len(matches) != 1:
            raise RuntimeError(f"combat idle v08 artifact drifted: {variant.variant_id}")
        artifact = matches[0]
        rendered_variants.append(
            {
                ...
                "source_revision": (
                    "v08" if variant.grip_mode == "one_handed" else "v06"
                ),
                "rendered_frame": {
    ...
    payload["combat_idle_down_weapon_variants_v08"] = {
        ...
        "rejected_one_hand_revision": {
            "revision": "v07",
            "reason": "blade_was_over_occluded_behind_torso",
        },
        "one_hand_correction": {
            ...
            "trajectory": "lateral_and_partly_behind_hero",
        },
        ...
        "locked_contract": {
            ...
            "one_hand_blade_lateral_and_partly_behind": True,
    ...
    payload["animation_contract"].update(
        {
            "combat_idle_down_active_stage": "weapon_variants_v08",
            "combat_idle_down_one_hand_revision": "v08",
...
def main() -> int:
    base_adapter.create_combat_idle_down_actions_v01 = create_weapon_stance_actions_v08
    base_adapter.render_pilot_combat_idle_down_v01 = render_weapon_stance_variants_v08
    base_adapter._write_contact_sheet_combat_idle_down_v01 = _write_contact_sheet_v08
    base_adapter._write_run_manifest_combat_idle_down_v01 = _write_manifest_v08
