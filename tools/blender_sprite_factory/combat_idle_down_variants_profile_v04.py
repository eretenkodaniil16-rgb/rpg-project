...
HUMAN_WARRIOR_M01_COMBAT_IDLE_DOWN_VARIANTS_V04 = CombatIdleDownVariantsProfileV02(
    ...
    variants=(
        CombatIdleDownVariantV02(
            variant_id="wide_low",
            display_name="Широкая низкая стойка",
            animation_id="combat_idle_wide_low_v04",
            pose=_pose(
                phase="wide_low",
                pelvis_x=0.01,
                pelvis_z=-0.060,
                ...
                hand_right_z_degrees=10.0,
            ),
        ),
        CombatIdleDownVariantV02(
            variant_id="wide_ready",
            display_name="Широкая боевая стойка",
            animation_id="combat_idle_wide_ready_v04",
            pose=_pose(
                phase="wide_ready",
                pelvis_x=0.0,
                pelvis_z=-0.065,
                ...
                upper_arm_right_x_degrees=-17.0,
                upper_arm_right_z_degrees=20.0,
        ...
        CombatIdleDownVariantV02(
            variant_id="wide_high",
            display_name="Широкая высокая стойка",
            animation_id="combat_idle_wide_high_v04",
            pose=_pose(
                phase="wide_high",
                pelvis_x=-0.01,
                pelvis_z=-0.070,
...
def load_combat_idle_down_variants_profile_v04(
    character_id: str,
) -> CombatIdleDownVariantsProfileV02:
    profile = HUMAN_WARRIOR_M01_COMBAT_IDLE_DOWN_VARIANTS_V04
    if character_id != profile.character_id:
        raise KeyError(
            f"No combat_idle_down variants v04 profile for character_id={character_id}"
        )
    if profile.revision != "v04":
        raise ValueError("Combat idle wide-side variants revision must be v04")
    expected_ids = ("wide_low", "wide_ready", "wide_high")
    if tuple(item.variant_id for item in profile.variants) != expected_ids:
        raise ValueError("Combat idle wide-side variant order drifted")

    baseline = HUMAN_WARRIOR_M01_COMBAT_IDLE_DOWN_V01.pose
    previous_right_arm_z = float("-inf")
    previous_hand_z = float("-inf")
    for item in profile.variants:
        ...
        if pose.upper_arm_left_z_degrees < 18.0:
            raise ValueError(f"Variant {item.variant_id} must send the left arm sideways")
        if pose.upper_arm_right_z_degrees < 14.0:
            raise ValueError(f"Variant {item.variant_id} must send the sword arm sideways")
        if pose.hand_right_z_degrees <= baseline.hand_right_z_degrees:
            raise ValueError(f"Variant {item.variant_id} must keep the blade outside the torso")
        if pose.upper_arm_right_z_degrees <= previous_right_arm_z:
            raise ValueError("Wide-side right-arm progression must be strictly increasing")
        if pose.hand_right_z_degrees <= previous_hand_z:
            raise ValueError("Wide-side blade progression must be strictly increasing")
        ...
        previous_right_arm_z = pose.upper_arm_right_z_degrees
        previous_hand_z = pose.hand_right_z_degrees
    return profile
