...
@dataclass(frozen=True)
class HairCrownProfileV12:
    revision: str
    proxy_revision: str
    mesh_name: str
    slices: tuple[HairCrownSliceV12, ...]

    def assert_valid(self) -> None:
        if self.revision != "v12" or self.proxy_revision != "v15":
            raise ValueError("Crown profile must match head v12 / proxy v15")
        if self.mesh_name != "hair_reference_crown_mesh":
            raise ValueError("Unexpected crown mesh identity")
        if len(self.slices) != 3:
            raise ValueError("Scalp coverage crown must keep front, middle and back slices")

        point_count = len(self.slices[0].points_xz)
        if point_count != 16:
            raise ValueError("Scalp coverage crown needs sixteen coarse contour points")
        ...
        for peak_index, left_valley, right_valley in ((3, 2, 4), (5, 4, 6), (7, 6, 8)):
            ...
            if peak - front[left_valley][1] < 0.075 or peak - front[right_valley][1] < 0.075:
                raise ValueError("Coverage fill must preserve broad physical crown waves")
        ...
        for slice_index, coverage_floor in enumerate(_COVERAGE_FLOORS):
            ...
            if min(coverage_z) < coverage_floor:
                raise ValueError("Crown valley is low enough for the scalp to break through")
        ...
        if max(rear_z) - min(rear_z) < 0.14:
            raise ValueError("Rear lower edge must preserve broad hanging masses")
        if min(rear_z) < 4.12:
            raise ValueError("Rear hair drops below the medium-length silhouette budget")


HUMAN_WARRIOR_M01_HAIR_CROWN_V12 = HairCrownProfileV12(
    revision="v12",
    proxy_revision="v15",
...
def load_hair_crown_profile_v12() -> HairCrownProfileV12:
    HUMAN_WARRIOR_M01_HAIR_CROWN_V12.assert_valid()
    return HUMAN_WARRIOR_M01_HAIR_CROWN_V12
