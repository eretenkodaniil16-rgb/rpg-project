...
├── source/
│   └── human_warrior_m01_proxy_v05.blend
├── raw/
├── frames/
│   ├── human_warrior_m01_idle_down_proxy_v05.png
│   ├── human_warrior_m01_idle_left_proxy_v05.png
│   ├── human_warrior_m01_idle_right_proxy_v05.png
│   ├── human_warrior_m01_idle_up_proxy_v05.png
│   └── human_warrior_m01_walk_down_f01–f06_proxy_v05.png
├── contact_sheet.png
└── run_manifest.json
```
...
или ноги нельзя. Камера, baseline, палитра, физические стороны экипировки и
ключи `walk_down` остаются зафиксированы. Номер и SHA-256 профиля записываются
в `run_manifest.json`.

## Текущий профиль головы

`head_profile.py` отделяет identity головы от уже принятого тела. Реальный
рендер первого прохода `head v01 / proxy_v04` подтвердил, что раздельная
геометрия головы работает во всех направлениях, но после уменьшения лицевая
плоскость остаётся слишком маленькой, а брови, глаза и рот схлопываются в
единый тёмный знак. В ракурсе `up` также не восстановилась недостающая высота.

Активный проход `head v02` создаёт кандидат `proxy_v05` и меняет только:

- сохраняет размер черепа и немного усиливает взрослую челюсть;
- открывает лицевую плоскость и увеличивает расстояния между бровями, глазами
  и ртом, чтобы они получили отдельные пиксельные ряды;
- уменьшает чрезмерный наклон бровей, сохраняя суровое выражение;
- увеличивает минимальную ширину глаз и рта до заметного после нормализации
  размера;
- разводит переднюю и заднюю верхние пряди по глубине, сохраняя их одинаковую
  высоту, чтобы восстановить экранную высоту `idle_down` и `idle_up` без
  увеличения головы;
- сохраняет `head v01 / proxy_v04` в коде как воспроизводимую предыдущую
  ревизию.

Профиль головы, его ревизия и SHA-256 записываются в `run_manifest.json`
отдельно от `silhouette v03`. Результат остаётся кандидатом до нового
реального рендера Blender 5.2 и сравнения трёхрядного листа.
