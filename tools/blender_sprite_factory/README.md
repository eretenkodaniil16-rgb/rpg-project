...
├── source/
│   └── human_warrior_m01_proxy_v04.blend
├── raw/
├── frames/
│   ├── human_warrior_m01_idle_down_proxy_v04.png
│   ├── human_warrior_m01_idle_left_proxy_v04.png
│   ├── human_warrior_m01_idle_right_proxy_v04.png
│   ├── human_warrior_m01_idle_up_proxy_v04.png
│   └── human_warrior_m01_walk_down_f01–f06_proxy_v04.png
├── contact_sheet.png
└── run_manifest.json
```
...
`head_profile.py` отделяет identity головы от уже принятого тела. Первый
проход `head v01` создаёт общий кандидат `proxy_v04` и меняет только:

- взрослую форму головы с отдельной более узкой челюстью;
- открытую лицевую плоскость вместо перекрывающей её цельной массы волос;
- отдельные верхнюю, передние, боковые и задние массы тёмных волнистых волос;
- физические уши и нос;
- минимальные геометрические брови, глаза и рот, рассчитанные на чтение после
  nearest-neighbor нормализации до 96×96;
- суровый наклон бровей;
- верхнюю переднюю прядь, которая должна вернуть недостающую высоту
  `idle_up`, не меняя камеру или масштаб тела.
