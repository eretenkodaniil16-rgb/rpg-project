...
- версионируемый профиль силуэта, отделённый от Blender API и анимаций;
...
- бинарный alpha, высоту около 78 px и базовую линию `y=91`;
...
├── source/
│   └── human_warrior_m01_proxy_v06.blend
├── raw/
├── frames/
│   ├── human_warrior_m01_idle_down_proxy_v06.png
│   ├── human_warrior_m01_idle_left_proxy_v06.png
│   ├── human_warrior_m01_idle_right_proxy_v06.png
│   ├── human_warrior_m01_idle_up_proxy_v06.png
│   └── human_warrior_m01_walk_down_f01–f06_proxy_v06.png
├── contact_sheet.png
└── run_manifest.json
```
...
технического среза — по одной группе проблем за итерацию.

## Текущий профиль силуэта

`silhouette_profile.py` содержит чистые типизированные параметры формы и не
импортирует Blender. Ревизия `v03` принята как техническая основа тела после
визуальной проверки реального Blender-рендера:
...
Маска `idle_down` занимает 42 px против 44 px у approved reference; вид со
спины почти совпадает по нижнему контуру. Остаточные выступы боковых эталонов
относятся к точной форме плаща, ножен и сумки, поэтому дальше расширять тело
или ноги нельзя. Камера, baseline, физические стороны экипировки и ключи
`walk_down` остаются зафиксированы. Номер и SHA-256 профиля записываются в
`run_manifest.json`.

## Текущий профиль головы

`head_profile.py` отделяет identity головы от уже принятого тела. Ревизии
`head v01 / proxy_v04` и `head v02 / proxy_v05` сохранены в коде как
воспроизводимые предыдущие проходы.

Активный визуальный кандидат `head v03 / proxy_v06` меняет только голову,
лицо, волосы и техническую палитру кожи:

- сохраняет без изменений размер и положение черепа из `head v02`;
- делает челюсть немного уже и выше, чтобы усилить взрослый подбородок;
- уменьшает нос, брови, глаза и рот как отдельные геометрические знаки и
  разводит их по высоте, чтобы избежать единой тёмной маски;
- разделяет волосы на верхнюю оболочку, передние, боковые, задние и затылочные
  массы;
- сохраняет отдельную переднюю прядь и добавляет связанную с основной массой
  фронтальную гряду, которая восстанавливает высоту заднего ракурса после
  настоящего поворота на 180°;
- использует более светлую холодную нейтрально-розовую техническую палитру
  кожи без изменения материалов брони и шарфа.

Контрольный реальный рендер Blender 5.2 LTS выполнен в отдельном run-каталоге
`30305197716_1`. Нормализованные `idle` занимают по высоте 76 px (`down`),
79 px (`left`), 79 px (`right`) и 77 px (`up`); baseline всех кадров остаётся
`y=91`. Профиль головы имеет SHA-256
`dc4576418e704cef7ad772f8664f09603de0053aaba4fbc5ee861cebdb7157b6`.

Это только кандидат для ручной визуальной проверки. `.blend`, текстуры и PNG
из run-каталога нельзя переносить в `source/`, `textures/approved/` или игровые
assets до отдельного утверждения contact sheet.
