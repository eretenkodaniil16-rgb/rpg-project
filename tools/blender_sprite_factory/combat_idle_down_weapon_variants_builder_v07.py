...
import combat_idle_down_weapon_variants_builder_v06 as previous_builder
...
ONE_HAND_LOW_V07_OBJECT_NAMES = (
    "combat_onehand_low_v07_blade",
    "combat_onehand_low_v07_highlight",
    "combat_onehand_low_v07_tip",
    "combat_onehand_low_v07_guard",
    "combat_onehand_low_v07_grip",
    "combat_onehand_low_v07_pommel",
)
ONE_HAND_READY_V07_OBJECT_NAMES = (
    "combat_onehand_ready_v07_blade",
    "combat_onehand_ready_v07_highlight",
    "combat_onehand_ready_v07_tip",
    "combat_onehand_ready_v07_guard",
    "combat_onehand_ready_v07_grip",
    "combat_onehand_ready_v07_pommel",
)
...
def _register_one_hand_part(
    ...
    *,
    ...
    registered["weapon_id"] = "sword_01_onehand_backside_v07"
    registered["weapon_part"] = weapon_part
    registered["weapon_state"] = "drawn"
    registered["weapon_variant_id"] = variant_id
    registered["combat_idle_weapon_variants_revision"] = "v07"
    registered["one_hand_trajectory"] = "side_and_behind_hero"
    registered.hide_render = True
    registered.hide_viewport = True
    return registered


def _build_one_hand_v07(
    context: factory.BuildContext,
    *,
    variant_id: str,
    prefix: str,
    ...
    highlight_material = previous_builder._ensure_highlight_material()
    ...
    highlight_offset = guard_lateral * 0.075 + factory.Vector((0.0, -0.020, 0.018))
    ...
    tip = previous_builder._pointed_tip(
        f"{prefix}_tip",
        blade_end,
        direction,
        0.135,
...
def create_weapon_stance_actions_v07(context: factory.BuildContext) -> None:
    ...
    for variant in profile.variants[:2]:
        ...
        if factory.bpy.data.actions.get(action_name) is not None:
            raise RuntimeError(f"combat idle v07 action already exists: {action_name}")
        action = action_builder._create_action(context, variant)
        action["profile_revision"] = "v07"
        action["one_hand_blade_side_and_behind"] = True
    ...
    low_parts = _build_one_hand_v07(
        context,
        variant_id=one_hand_low.variant_id,
        prefix="combat_onehand_low_v07",
    )
    ...
    factory.bpy.context.scene.frame_set(1)
    factory.bpy.context.view_layer.update()
    ready_parts = _build_one_hand_v07(
        context,
        variant_id=one_hand_ready.variant_id,
        prefix="combat_onehand_ready_v07",
    )
    ...
    scene["combat_idle_weapon_variants_revision"] = "v07"
    scene["combat_idle_weapon_variant_count"] = len(profile.variants)
    scene["combat_idle_one_hand_low_v07_object_count"] = len(low_parts)
    scene["combat_idle_one_hand_ready_v07_object_count"] = len(ready_parts)
    scene["combat_idle_one_hand_separate_pose_fitted_modules"] = True
    scene["combat_idle_one_hand_side_and_behind"] = True
