...
import blender_sprite_factory_combat_idle_down_v01 as previous_adapter
from combat_idle_down_variants_builder_v02 import (
    create_combat_idle_down_variant_actions_v02,
)
...
BASE_RENDER = previous_adapter.render_pilot_combat_idle_down_v01
BASE_CONTACT_SHEET = previous_adapter._write_contact_sheet_combat_idle_down_v01
BASE_WRITE_MANIFEST = previous_adapter._write_run_manifest_combat_idle_down_v01
PROFILE_PATH = SCRIPT_DIR / "combat_idle_down_variants_profile_v02.py"
BUILDER_PATH = SCRIPT_DIR / "combat_idle_down_variants_builder_v02.py"
COMPARISON_SHEET_NAME = "combat_idle_down_variants_v02.png"


def _calibrate_idle_down(
    context: factory.BuildContext,
    run_dir: Path,
) -> factory.FramingCalibration:
    config = context.config
    temp_root = run_dir / "_combat_idle_v02_calibration"
    raw_dir = temp_root / "raw"
    frame_dir = temp_root / "frames"
    raw_dir.mkdir(parents=True)
    frame_dir.mkdir(parents=True)
    idle_action = factory.bpy.data.actions[f"{config.character_id}_idle"]
    previous_adapter._set_combat_weapon_state(False)
    ...
    try:
        _, calibration = factory._render_frame(
            context,
            animation_id="idle_calibration",
            direction="down",
            frame_number=1,
            raw_dir=raw_dir,
            frame_dir=frame_dir,
            output_name="combat_idle_v02_calibration.png",
            fixed_scale=None,
            fixed_center_x=None,
        )
        return calibration
...
def render_pilot_combat_idle_down_variants_v02(
    context: factory.BuildContext,
    run_dir: Path,
    ...
    profile = load_combat_idle_down_variants_profile_v02(config.character_id)
    calibration = _calibrate_idle_down(context, run_dir)
    raw_dir = run_dir / "raw"
    frame_dir = run_dir / "frames"
    idle_action = factory.bpy.data.actions[f"{config.character_id}_idle"]

    try:
        previous_adapter._set_combat_weapon_state(True)
        for variant in profile.variants:
            action_name = f"{config.character_id}_{variant.animation_id}"
            action = factory.bpy.data.actions.get(action_name)
            if action is None:
                raise RuntimeError(
                    f"combat_idle_down variants v02 cannot find action {action_name}"
                )
...
def _candidate_paths(
    ...
    animation_ids = (
        "combat_idle",
        "combat_idle_center_low",
        "combat_idle_center_mid",
        "combat_idle_diagonal_guard",
    )
    paths: list[Path] = []
    for animation_id in animation_ids:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"combat_idle_down variants v02 requires one {animation_id} frame, "
                f"got {len(matches)}"
            )
...
def _write_variant_comparison_sheet(
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_combat_idle_down_variants_v02",
        width=width,
        height=height,
        alpha=True,
        float_buffer=False,
...
def _write_contact_sheet_combat_idle_down_variants_v02(
    config: object,
    artifacts: list[factory.FrameArtifact],
    output_path: Path,
) -> Path:
...
def _write_run_manifest_combat_idle_down_variants_v02(
    context: factory.BuildContext,
    run_dir: Path,
    run_id: str,
    blend_path: Path,
    ...
    if not comparison_path.is_file():
        raise RuntimeError("combat_idle_down variants v02 comparison sheet is missing")
    ...
    for variant in profile.variants:
        ...
        if action is None or action.get("variant_id") != variant.variant_id:
            raise RuntimeError(
                f"combat_idle_down variants v02 action drifted: {variant.variant_id}"
            )
        matches = [
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"combat_idle_down variants v02 artifact drifted: {variant.variant_id}"
            )
        artifact = matches[0]
    ...
    payload["combat_idle_down_variants_v02"] = {
    ...
    payload["animation_contract"].update(
        {
            "combat_idle_down_active_stage": "variants_v02",
    ...
    payload["appearance_candidate"]["status"] = (
        "artist_approved_appearance_and_walk_set_with_combat_idle_down_"
        "variants_v02_pending_selection"
    )
...
def main() -> int:
    previous_adapter.create_combat_idle_down_actions_v01 = (
        create_combat_idle_down_variant_actions_v02
    )
    previous_adapter.render_pilot_combat_idle_down_v01 = (
        render_pilot_combat_idle_down_variants_v02
    )
    previous_adapter._write_contact_sheet_combat_idle_down_v01 = (
        _write_contact_sheet_combat_idle_down_variants_v02
    )
    previous_adapter._write_run_manifest_combat_idle_down_v01 = (
        _write_run_manifest_combat_idle_down_variants_v02
    )
