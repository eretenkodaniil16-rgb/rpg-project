...
def _density_for_hair(
    detail: HeadDetailProfile,
    scale: tuple[float, float, float],
) -> tuple[int, int]:
    density = (
        detail.hair_main_density
        if max(scale) >= 0.30
        else detail.hair_lock_density
    )
    return density.segments, density.rings


def _build_head_and_hair_v04(context: factory.BuildContext) -> None:
    head = context.head
    detail = load_head_detail_profile_v04(context.config.character_id)
    ...
    for face_part in head.brows + head.eyes + (head.mouth,):
        _register_box(
            context,
            DetailedBoxPart(face_part, "hair"),
            "head",
        )
...
def main() -> int:
    factory.load_head_profile = load_head_profile_v04
    factory._build_head_and_hair = _build_head_and_hair_v04
    factory._write_run_manifest = _write_run_manifest_v04
    return factory.main()
