...
DIAGNOSTIC_SCENE_KEY = "attack_sword_twohand_left_tail_diagnostic_v21"
CONTACT_SHEET_NAME = "attack_sword_01_twohand_left_tail_diagnostic_v21.png"
...
def _projection_target_direction(
    ...
    *,
    ...
    if source_projection <= 1.0e-6:
        raise RuntimeError(
            "two-hand left tail diagnostic source projection is degenerate"
        )
...
def _diagnose_frame(
    ...
    *,
    ...
    target_rotations = _capture_arm_rotations(context, target_frame)
    source_rotations = _capture_arm_rotations(context, source_frame)
    ...
    try:
                    ...
                    try:
                        ...
                        artifact, _ = export_adapter._render_candidate(
                            ...
                            output_name=(
                                ...
                                f"twohand_left_tail_diagnostic_v21_"
                        ...
                        diagnostics.append(diagnostic)
                        print(
                            "ATTACK_SWORD_TWOHAND_LEFT_TAIL_"
        ...
        if selected is None or selected_artifact is None:
            raise RuntimeError(
                "two-hand left tail diagnostic found no safe candidate for "
        ...
        print(
            "ATTACK_SWORD_TWOHAND_LEFT_TAIL_DIAGNOSTIC_V21_SELECTED="
            f"frame:{target_frame};"
            f"blend:{float(selected['arm_blend']):.2f};"
            f"projection:{float(selected['applied_projection']):.2f};"
        ...
        return selected_artifact, result
    finally:
...
def _render_diagnostic(
    ...
    if action is None:
        raise RuntimeError(
            f"two-hand left tail diagnostic action is missing: {TARGET_ACTION_ID}"
        )
    ...
    try:
        ...
        for target_frame in TARGET_FRAMES:
            artifact, result = _diagnose_frame(
                context,
                run_dir,
        ...
        payload = {
            "revision": TWOHAND_LEFT_TAIL_DIAGNOSTIC_REVISION,
...
def _write_four_frame_sheet(
    ...
    if [item.frame_number for item in ordered] != list(TARGET_FRAMES):
        raise RuntimeError(
            "two-hand left tail diagnostic expected frames f05-f08"
        )
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_attack_sword_twohand_left_tail_diagnostic_v21",
        width=width,
        height=height,
        alpha=True,
        float_buffer=False,
...
def _write_manifest(
    ...
    payload["onehand_left_changed"] = False
    payload["twohand_left_f02_f04_changed"] = False
