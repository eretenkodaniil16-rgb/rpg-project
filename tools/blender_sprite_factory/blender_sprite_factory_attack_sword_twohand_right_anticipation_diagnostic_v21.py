...
from attack_sword_directional_cycle_builder_v21_pass17 import (
...
DIAGNOSTIC_SCENE_KEY = (
    "attack_sword_twohand_right_anticipation_diagnostic_v21"
)
CONTACT_SHEET_NAME = (
    "attack_sword_01_twohand_right_anticipation_diagnostic_v21.png"
)
...
def _projection_target_direction(
    ...
    *,
    ...
    if source_projection <= 1.0e-6:
        raise RuntimeError(
            "two-hand right anticipation diagnostic source projection is degenerate"
        )
...
def _render_diagnostic(
    ...
    if action is None:
        raise RuntimeError(
            f"two-hand right anticipation action is missing: {TARGET_ACTION_ID}"
        )
    ...
    try:
        ...
        for source_frame in SOURCE_FRAME_CANDIDATES:
            source_rotations = _capture_arm(context, int(source_frame))
            ...
            for arm_blend in ARM_BLEND_CANDIDATES:
                ...
                saved_basis = {
                    obj.name: obj.matrix_basis.copy() for obj in objects
                }
                    ...
                    for offset_degrees in ANGLE_OFFSET_CANDIDATES:
                        ...
                        try:
                            ...
                            artifact, _ = export_adapter._render_candidate(
                                context,
                                animation_id=(
                                    "attack_sword_01_twohand_right_"
                                    "anticipation_diagnostic_v21"
                                ),
                                ...
                                output_name=(
                                    f"{config.character_id}_attack_sword_01_"
                                    "twohand_right_anticipation_"
                                    f"diagnostic_v21_f{TARGET_FRAME:02d}_"
                            ...
                            diagnostic["accepted"] = (
                                not touched
                                if REQUIRE_ZERO_EDGE_ALPHA
                                else True
                            )
                            diagnostics.append(diagnostic)
                            print(
                                "ATTACK_SWORD_TWOHAND_RIGHT_ANTICIPATION_"
                                "DIAGNOSTIC_V21_ATTEMPT="
                                f"source:{int(source_frame)};"
        ...
        if selected is None or selected_artifact is None:
            raise RuntimeError(
                "two-hand right anticipation diagnostic found no safe "
                f"candidate: {diagnostics}"
            )

        payload = {
            "revision": TWOHAND_RIGHT_ANTICIPATION_DIAGNOSTIC_REVISION,
...
def _write_single_frame_sheet(
    ...
    if len(artifacts) != 1 or artifacts[0].frame_number != TARGET_FRAME:
        raise RuntimeError(
            "two-hand right anticipation diagnostic expected one f03 frame"
        )
    source = artifacts[0].output_path
    image = factory.bpy.data.images.load(str(source), check_existing=False)
    try:
        pixels = tuple(image.pixels[:])
        sheet = factory.bpy.data.images.new(
            "human_warrior_m01_attack_sword_twohand_right_anticipation_diagnostic_v21",
...
def _write_manifest(
    ...
    payload["onehand_right_changed"] = False
    payload["twohand_right_f01_changed"] = False
    payload["twohand_right_f02_action_changed"] = True
    payload["up_actions_changed"] = False
