...
from attack_sword_directional_cycle_correction_v21_pass52 import (
    ANGLE_OFFSET_CANDIDATES,
    ARM_BLEND_CANDIDATES,
    CORRECTION_PASS,
    DEPTH_BRANCH_CANDIDATES,
    GUARD_REFERENCE_FRAME,
...
DIAGNOSTIC_SCENE_KEY = "attack_sword_twohand_up_f08_review_v21_pass52"
CONTACT_SHEET_NAME = "attack_sword_01_twohand_up_f08_review_v21_pass52.png"
CORRECTION_PATH = SCRIPT_DIR / "attack_sword_directional_cycle_correction_v21_pass52.py"
...
def _candidate_sort_key(candidate: dict[str, object]) -> tuple[object, ...]:
    from_f07 = float(candidate["continuity_from_selected_f07_rms_degrees"])
    to_guard = float(candidate["deviation_from_guard_rms_degrees"])
    return (
        max(from_f07, to_guard),
        to_guard,
        abs(from_f07 - to_guard),
        from_f07 + to_guard,
...
def _select_review_candidates(
    ...
    for candidate in ordered:
        ...
        if key in seen_arm_profiles:
            continue
        seen_arm_profiles.add(key)
    ...
    for candidate in ordered:
        ...
        if key in seen_full_profiles:
            continue
        seen_full_profiles.add(key)
        selected.append(candidate)
        if len(selected) == REVIEW_VARIANT_COUNT:
            break
    return tuple(selected)
...
def _evaluate_candidates(
    ...
    *,
    ...
    selected_f07_rotations: dict[str, object],
    ...
    try:
        for arm_blend in ARM_BLEND_CANDIDATES:
            for source_order, source_code in enumerate(SOURCE_POSE_CODES):
                ...
                from_f07 = pass34_adapter._arm_rms_degrees(
                    selected_f07_rotations,
                    candidate_pose,
                )
                to_guard = pass34_adapter._arm_rms_degrees(
                    candidate_pose,
                    guard_rotations,
                )
                accepted_count = 0
                for candidate in evaluated:
                    ...
                    enriched.update(
                        {
                            ...
                            "source_pose_label": SOURCE_POSE_LABELS[
                                int(source_code)
                            ],
                            "continuity_from_selected_f07_rms_degrees": from_f07,
                            "deviation_from_guard_rms_degrees": to_guard,
                            "continuity_score": from_f07 + to_guard,
                    ...
                    accepted_count += 1
                safe_counts[
                    f"{SOURCE_POSE_LABELS[int(source_code)]}_"
...
def _render_candidate(
    ...
    *,
    ...
    try:
        output_name = (
            f"{config.character_id}_attack_sword_01_twohand_up_f08_"
            f"review_v21_pass52_c{variant_index:02d}_"
        ...
        artifact, _ = export_adapter._render_candidate(
            context,
            animation_id=(
                "attack_sword_01_twohand_up_f08_review_v21_pass52_"
        ...
        metadata = {
            **candidate,
            ...
            "accepted_by_final_boundary_contract": not touched,
            "applied_arm_deltas_degrees": applied_arm_deltas,
        }
        print(
            "ATTACK_SWORD_TWOHAND_UP_F08_REVIEW_V21_PASS52_CANDIDATE="
            ...
            f"from_f07:{float(candidate['continuity_from_selected_f07_rms_degrees']):.3f};"
            f"to_guard:{float(candidate['deviation_from_guard_rms_degrees']):.3f};"
        ...
        if (
            ...
            raise RuntimeError(
                "two-hand up f08 pass52 candidate touched canvas edges: "
...
def _render_review(
    ...
    if action is None:
        raise RuntimeError(
            "two-hand up f08 pass52 review action is missing: "
            f"{TARGET_ACTION_ID}"
        )
    ...
    try:
        ...
        guard_rotations = pass29_adapter._capture_arm(
            context,
            GUARD_REFERENCE_FRAME,
        )
        original_f07_rotations = pass29_adapter._capture_arm(
            context,
            PREVIOUS_REFERENCE_FRAME,
        )
        ...
        source_rotations_by_code = {
            7: selected_f07_rotations,
            1: guard_rotations,
            6: original_f06_rotations,
            5: pass29_adapter._capture_arm(context, 5),
        }
        candidates, safe_counts = _evaluate_candidates(
            context,
            target_rotations=target_rotations,
            selected_f07_rotations=selected_f07_rotations,
            guard_rotations=guard_rotations,
            source_rotations_by_code=source_rotations_by_code,
        )
        selected = _select_review_candidates(candidates)
        if not selected:
            raise RuntimeError(
                "two-hand up f08 pass52 found no geometry-safe settle "
        ...
        print(
            "ATTACK_SWORD_TWOHAND_UP_F08_REVIEW_V21_PASS52="
...
def _write_review_sheet(
    ...
    if not artifacts:
        raise RuntimeError("two-hand up f08 pass52 review sheet is empty")
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_attack_sword_twohand_up_f08_review_v21_pass52",
        width=width,
        height=tile_height,
        alpha=True,
        float_buffer=False,
