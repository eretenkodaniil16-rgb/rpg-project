...
import blender_sprite_factory_attack_sword_directional_cycle_v21_pass05 as pass05_adapter
...
CORRECTION_PATH = (
    SCRIPT_DIR / "attack_sword_directional_cycle_correction_v21_pass09.py"
)
BUILDER_PATH = (
    SCRIPT_DIR / "attack_sword_directional_cycle_builder_v21_pass09.py"
)
CONTACT_SHEET_NAME = "attack_sword_01_directional_cycle_v21.png"
BASE_RENDER_FRAME_PASS05 = pass05_adapter._render_frame_v21_pass05
BASE_WRITE_MANIFEST_PASS05 = pass05_adapter._write_manifest_v21_pass05
...
def _target_direction_v21_pass09(
    ...
    if source_projection <= 1.0e-6:
        raise RuntimeError(
            "attack sword directional v21 pass09 source projection is degenerate"
        )
    target_projection = min(source_projection, SELECTED_SCREEN_PROJECTION)
    angle = (
        math.atan2(current_y, current_x)
        + math.radians(SELECTED_WEAPON_OFFSET_DEGREES)
    )
...
def _render_frame_v21_pass09(
    ...
    *,
    ...
    if not _is_target_frame(animation_id, direction, frame_number):
        return BASE_RENDER_FRAME_PASS05(
            context,
    ...
    if action is None:
        raise RuntimeError(
            "attack sword directional v21 pass09 target action is missing"
        )
    if (
        action.get("directional_twohand_left_windup_revision")
        != TWOHAND_LEFT_WINDUP_REVISION
    ):
        raise RuntimeError(
            "attack sword directional v21 pass09 action metadata drifted"
        )
    if not math.isclose(
        float(action.get("directional_twohand_left_windup_arm_blend", -1.0)),
        SELECTED_ARM_BLEND,
        abs_tol=1.0e-9,
    ):
        raise RuntimeError(
            "attack sword directional v21 pass09 arm blend drifted"
        )
    ...
    target_direction, source_projection, applied_projection = (
        _target_direction_v21_pass09(current_direction)
    )
    ...
    try:
        ...
        if clearance < MIN_HEAD_CLEARANCE_PIXELS:
            raise RuntimeError(
                "attack sword directional v21 pass09 head clearance drifted: "
                f"{clearance:.3f}px"
            )
        if margin < MIN_CAMERA_MARGIN_PIXELS:
            raise RuntimeError(
                "attack sword directional v21 pass09 camera margin drifted: "
                f"{margin:.3f}px"
            )
        ...
        if REQUIRE_ZERO_EDGE_ALPHA and touched:
            raise RuntimeError(
                "attack sword directional v21 pass09 target frame touched export "
                f"edges: {touched}"
            )
    finally:
    ...
    metrics[key] = {
        ...
        "pass09_arm_blend": SELECTED_ARM_BLEND,
        "pass09_screen_projection": float(applied_projection),
    }
    ...
    print(
        "ATTACK_SWORD_DIRECTIONAL_V21_PASS09_SELECTED="
        f"{key};"
...
def _write_manifest_v21_pass09(
    context: factory.BuildContext,
    run_dir: Path,
    run_id: str,
    blend_path: Path,
    ...
    manifest_path = BASE_WRITE_MANIFEST_PASS05(
        context,
        run_dir,
        run_id,
        blend_path,
    ...
    if action is None:
        raise RuntimeError(
            "attack sword directional v21 pass09 manifest action is missing"
        )
    if (
        action.get("directional_twohand_left_windup_revision")
        != TWOHAND_LEFT_WINDUP_REVISION
        ...
        raise RuntimeError(
            "attack sword directional v21 pass09 manifest metadata drifted"
        )
    ...
    if target_key not in metrics:
        raise RuntimeError(
            "attack sword directional v21 pass09 target metrics are missing"
        )

    payload["attack_sword_directional_cycle_v21_pass09"] = {
        "correction_pass": CORRECTION_PASS,
        "twohand_left_windup_revision": TWOHAND_LEFT_WINDUP_REVISION,
        ...
        "screen_projection_reduced_without_scaling": True,
    ...
    payload.setdefault("animation_contract", {}).update(
        {
            "attack_sword_01_current_stage": (
                "directional_full_cycle_v21_pass09"
            ),
            "attack_sword_01_left_twohand_windup_fixed": True,
...
def main() -> int:
    pass05_adapter._render_frame_v21_pass05 = _render_frame_v21_pass09
    pass05_adapter.create_attack_sword_directional_cycle_actions_v21_pass05 = (
        create_attack_sword_directional_cycle_actions_v21_pass09
    )
    pass05_adapter._write_manifest_v21_pass05 = _write_manifest_v21_pass09
