...
class CombatIdleDirectionalWeaponV12Tests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.profile = load_combat_idle_directional_weapon_profile_v12("human_warrior_m01")
        cls.tool_root = Path(__file__).resolve().parents[1]
        cls.builder_source = (cls.tool_root / "combat_idle_directional_weapon_builder_v12.py").read_text(encoding="utf-8")
        cls.adapter_source = (cls.tool_root / "blender_sprite_factory_combat_idle_directional_weapon_v12.py").read_text(encoding="utf-8")

    def test_only_left_right_up_onehand_projections_are_corrected(self) -> None:
        self.assertEqual(self.profile.revision, "v12")
        self.assertEqual(tuple(item.direction for item in self.profile.corrected_onehand_directions), ("left", "right", "up"))
        self.assertEqual(self.profile.preserved_down_source, "combat_idle_onehand_ready_directional_v11_down")
        self.assertEqual(self.profile.preserved_twohand_source, "combat_idle_twohand_center_high_directional_v11")
    ...
    def test_v12_is_rejected_historical_source_for_v13(self) -> None:
        ...
        self.assertIn("Rejected formal-only directional correction: blender_sprite_factory_combat_idle_directional_weapon_v12.py", launcher)
        self.assertIn("render-combat-idle-directional-weapon-v12 (rejected: left blade remained occluded; right blade too horizontal)", workflow)
