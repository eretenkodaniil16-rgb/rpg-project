from __future__ import annotations

import ast
import unittest
from pathlib import Path

from attack_sword_directional_cycle_correction_v21_pass31 import (
    ARM_BLEND_CANDIDATES,
    CORRECTION_PASS,
    MAX_ABS_WEAPON_OFFSET_DEGREES,
    PREFERRED_ABS_WEAPON_OFFSET_DEGREES,
    REJECTED_REVIEW_ARTIFACT_ID,
    REJECTED_REVIEW_RUN_ID,
    REVIEW_VARIANT_COUNT,
    SOURCE_FRAME_CANDIDATES,
    TARGET_ACTION_ID,
    TARGET_DIRECTION,
    TARGET_FRAME,
    TARGET_GRIP_ID,
    TWOHAND_UP_F01_CENTRAL_REVIEW_REVISION,
)


class AttackSwordTwohandUpF01CentralReviewV21Pass31Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.tool_root = Path(__file__).resolve().parents[1]
        cls.adapter_name = (
            "blender_sprite_factory_attack_sword_twohand_up_"
            "f01_central_review_v21_pass31.py"
        )
        cls.adapter_source = (
            cls.tool_root / cls.adapter_name
        ).read_text(encoding="utf-8")
        cls.workflow_source = (
            cls.tool_root.parents[1]
            / ".github"
            / "workflows"
            / "validate-human-warrior-attack-directional-v21.yml"
        ).read_text(encoding="utf-8")

    def test_central_review_contract(self) -> None:
        self.assertEqual(CORRECTION_PASS, "v21_pass31")
        self.assertEqual(
            TWOHAND_UP_F01_CENTRAL_REVIEW_REVISION,
            "twohand_up_f01_central_high_grip_review_v21_pass31",
        )
        self.assertEqual(TARGET_ACTION_ID, "attack_sword_01_twohand_up_v21")
        self.assertEqual(TARGET_GRIP_ID, "twohand_center_high")
        self.assertEqual(TARGET_DIRECTION, "up")
        self.assertEqual(TARGET_FRAME, 1)
        self.assertEqual(SOURCE_FRAME_CANDIDATES[0], 2)
        self.assertEqual(min(ARM_BLEND_CANDIDATES), 0.30)
        self.assertEqual(max(ARM_BLEND_CANDIDATES), 1.00)
        self.assertLessEqual(MAX_ABS_WEAPON_OFFSET_DEGREES, 48.0)
        self.assertLessEqual(PREFERRED_ABS_WEAPON_OFFSET_DEGREES, 24.0)
        self.assertEqual(REVIEW_VARIANT_COUNT, 4)
        self.assertEqual(REJECTED_REVIEW_RUN_ID, 30851687993)
        self.assertEqual(REJECTED_REVIEW_ARTIFACT_ID, 8871022374)

    def test_adapter_prefers_small_weapon_offset_and_is_non_destructive(self) -> None:
        ast.parse(self.adapter_source)
        self.assertIn("_central_sort_key", self.adapter_source)
        self.assertIn("_select_review_candidates", self.adapter_source)
        self.assertIn("MAX_ABS_WEAPON_OFFSET_DEGREES", self.adapter_source)
        self.assertIn("PREFERRED_ABS_WEAPON_OFFSET_DEGREES", self.adapter_source)
        self.assertIn("_render_reference", self.adapter_source)
        self.assertIn("_render_up_candidate", self.adapter_source)
        self.assertIn("twohand_up_action_data_changed", self.adapter_source)
        self.assertNotIn("point.co[1] =", self.adapter_source)
        self.assertNotIn("vertex.co =", self.adapter_source)
        self.assertNotIn("root.location", self.adapter_source)
        self.assertNotIn("obj.scale", self.adapter_source)
        self.assertNotIn("hide_render = True", self.adapter_source)

    def test_workflow_uses_pass31_and_keeps_full_entrypoint(self) -> None:
        self.assertIn(self.adapter_name, self.workflow_source)
        self.assertIn("twohand up f01 central", self.workflow_source.lower())
        self.assertIn(
            "blender_sprite_factory_attack_sword_directional_cycle_v21_pass28.py",
            self.workflow_source,
        )
        self.assertIn("full directional cycle", self.workflow_source)


if __name__ == "__main__":
    unittest.main()
