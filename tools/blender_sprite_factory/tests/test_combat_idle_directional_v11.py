...
class CombatIdleDirectionalV11Tests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.adapter_source = (cls.tool_root / "blender_sprite_factory_combat_idle_directional_v11.py").read_text(encoding="utf-8")
    ...
    def test_selected_sources_and_direction_order_are_locked(self) -> None:
        ...
        self.assertEqual(tuple(candidate.candidate_id for candidate in self.profile.candidates), ("onehand_ready", "twohand_center_high"))
        self.assertEqual(tuple(candidate.source_animation_id for candidate in self.profile.candidates), ("combat_idle_onehand_ready_cycle_v10", "combat_idle_twohand_center_high_cycle_v10"))
    ...
    def test_approved_down_pixels_are_enforced(self) -> None:
        self.assertIn('rendered["down"].output_path.read_bytes() != approved.output_path.read_bytes()', self.adapter_source)
    ...
    def test_v11_remains_rejected_history_under_active_v13(self) -> None:
        ...
        self.assertIn("Rejected raw directional rotation: blender_sprite_factory_combat_idle_directional_v11.py", launcher)
        self.assertIn("render-combat-idle-directional-v11 (rejected: raw rotation caused one-hand occlusion and boundary touches)", workflow)
