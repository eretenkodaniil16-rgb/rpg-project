...
class CombatIdleDirectionalWeaponV13Tests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.profile = load_combat_idle_directional_weapon_profile_v13("human_warrior_m01")
        cls.tool_root = Path(__file__).resolve().parents[1]
        cls.builder = (cls.tool_root / "combat_idle_directional_weapon_builder_v13.py").read_text(encoding="utf-8")
        cls.adapter = (cls.tool_root / "blender_sprite_factory_combat_idle_directional_weapon_v13.py").read_text(encoding="utf-8")

    def test_only_onehand_left_and_right_are_corrected(self) -> None:
        self.assertEqual(self.profile.revision, "v13")
        self.assertEqual(tuple(item.direction for item in self.profile.corrected_sides), ("left", "right"))
        ...
        for item in self.profile.corrected_sides:
            ...
            self.assertLessEqual(max(abs(v) for v in item.anchor_offset), 0.35)
    ...
    def test_adapter_locks_down_up_and_twohand_pixels(self) -> None:
        ...
        self.assertIn("candidate_id == \"twohand_center_high\" or direction in profile.locked_directions", self.adapter)
    ...
    def test_side_candidates_have_visibility_and_boundary_contracts(self) -> None:
        ...
        self.assertIn("corrected_onehand_side_candidates_require_manual_review", self.adapter)

    def test_active_launcher_and_workflow_use_v13(self) -> None:
        ...
        self.assertIn("blender_sprite_factory_combat_idle_directional_weapon_v13.py", workflow)
