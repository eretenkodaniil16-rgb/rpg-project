...
class WalkHistoricalEntrypointsTests(unittest.TestCase):
    ...
    def test_windows_launcher_advances_to_walk_up_v02_adapter(self) -> None:
        self.assertIn(
            '$FactoryScript = Join-Path $ToolRoot "blender_sprite_factory_walk_up_v02.py"',
            self.launcher,
        )

    def test_ci_uses_walk_up_v02_real_blender_render(self) -> None:
        self.assertIn("render-walk-up-v02:", self.workflow)
        self.assertIn(
            "--python tools/blender_sprite_factory/blender_sprite_factory_walk_up_v02.py",
            self.workflow,
        )
        self.assertIn(
