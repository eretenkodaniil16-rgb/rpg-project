...
class WalkHistoricalEntrypointsTests(unittest.TestCase):
    ...
    def test_windows_launcher_advances_to_centered_variants_v03_adapter(self) -> None:
        self.assertIn(
            '$FactoryScript = Join-Path $ToolRoot "blender_sprite_factory_combat_idle_down_variants_v03.py"',
            self.launcher,
        )
        self.assertIn(
    ...
    def test_ci_uses_centered_variants_v03_real_blender_render(self) -> None:
        self.assertIn("render-combat-idle-down-variants-v03:", self.workflow)
        self.assertIn(
            "--python tools/blender_sprite_factory/blender_sprite_factory_combat_idle_down_variants_v03.py",
            self.workflow,
        )
        self.assertIn(
            "human_warrior_m01_proxy_v25_appearance_v03_walk_down_v04_walk_left_v01_walk_right_v01_walk_up_v02_combat_idle_down_variants_v03_",
            self.workflow,
        )
