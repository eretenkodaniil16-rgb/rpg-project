...
class CombatIdleDownCyclesV10Tests(unittest.TestCase):
    ...
    def test_frame_one_preserves_the_approved_pose(self) -> None:
        expected_sources = (self.stances.variants[1], self.stances.variants[3])
        for cycle, source in zip(self.profile.cycles, expected_sources):
            with self.subTest(cycle=cycle.cycle_id):
                self.assertEqual(
    ...
    def test_lower_body_is_planted_in_both_cycles(self) -> None:
        ...
        for cycle in self.profile.cycles:
            ...
            for frame in cycle.frames[1:]:
                with self.subTest(cycle=cycle.cycle_id, frame=frame.pose.frame):
                    for attribute in lower_body_attributes:
                        self.assertEqual(
    ...
    def test_v10_remains_approved_source_under_active_v12_stage(self) -> None:
