...
class AttackSwordTwohandUpF03ReviewV21Pass41Tests(unittest.TestCase):
    @classmethod
    ...
    def test_fine_offset_contract(self) -> None:
        self.assertEqual(CORRECTION_PASS, "v21_pass41")
        self.assertEqual(
            TWOHAND_UP_F03_FINE_OFFSET_REVIEW_REVISION,
            "twohand_up_f03_upward_arc_fine_offset_review_v21_pass41",
        )
        self.assertEqual(FINE_SCREEN_PROJECTION_CANDIDATES, (0.50, 0.45))
        self.assertEqual(
            FINE_ANGLE_OFFSET_CANDIDATES,
            (-4.0, -6.0, -8.0, -10.0),
        )
        ...
        self.assertEqual(
            ...
            [0.50, 0.50, 0.50, 0.45, 0.45, 0.45],
        )
        self.assertEqual(
            [item["weapon_offset_degrees"] for item in TARGETED_OFFSET_SPECS],
            [-6.0, -8.0, -10.0, -4.0, -6.0, -8.0],
        )
        self.assertTrue(RENDER_EDGE_TOUCHING_CANDIDATES_FOR_REVIEW)
        self.assertTrue(REQUIRE_ZERO_EDGE_ALPHA_FOR_ACCEPTANCE)
        self.assertEqual(SOURCE_PASS40_RUN_ID, 30858909679)
        self.assertEqual(SOURCE_PASS40_ARTIFACT_ID, 8873680973)

    def test_adapter_changes_only_weapon_search_contract(self) -> None:
        ast.parse(self.adapter_source)
        self.assertIn("pass29_adapter.SCREEN_PROJECTION_CANDIDATES", self.adapter_source)
        self.assertIn("pass29_adapter.ANGLE_OFFSET_CANDIDATES", self.adapter_source)
        self.assertIn("TARGETED_OFFSET_SPECS", self.adapter_source)
        self.assertIn("_restore_pass40_contract", self.adapter_source)
        self.assertIn("_write_manifest_v21_pass41", self.adapter_source)
    ...
    def test_workflow_uses_pass41_and_retains_pass40(self) -> None:
        self.assertIn(self.adapter_name, self.workflow_source)
        self.assertIn("fine f03 offset review", self.workflow_source.lower())
        self.assertIn(
            "blender_sprite_factory_attack_sword_twohand_up_"
