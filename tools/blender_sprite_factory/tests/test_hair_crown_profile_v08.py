...
class HairCrownProfileV08Tests(unittest.TestCase):
    ...
    def test_front_contour_has_large_asymmetric_waves(self) -> None:
    ...
    def test_front_hairline_integrates_separate_forelock_root(self) -> None:
        hairline = self.profile.slices[0].points_xz[12:16]
        lowest = min(hairline, key=lambda point: point[1])
        self.assertLess(lowest[0], 0.0)
        self.assertLess(lowest[1], 4.42)
        self.assertGreater(max(point[1] for point in hairline), 4.50)
