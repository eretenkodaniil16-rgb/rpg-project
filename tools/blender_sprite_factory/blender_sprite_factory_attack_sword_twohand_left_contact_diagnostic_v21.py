...
def _projection_target_direction(
    ...
    *,
    ...
    if source_projection <= 1.0e-6:
        raise RuntimeError(
            "two-hand left contact diagnostic source projection is degenerate"
        )
...
def _render_diagnostic(
    ...
    if action is None:
        raise RuntimeError(
            f"two-hand left contact diagnostic action is missing: {TARGET_ACTION_ID}"
        )
    ...
    try:
                    ...
                    try:
                        ...
                        artifact, _ = export_adapter._render_candidate(
                            ...
                            output_name=(
                                ...
                                f"twohand_left_contact_diagnostic_v21_"
                                f"f04_proxy_{context.proxy_revision}.png"
                            ),
                        ...
                        diagnostics.append(diagnostic)
                        print(
                            "ATTACK_SWORD_TWOHAND_LEFT_CONTACT_"
        ...
        if selected is None or selected_artifact is None:
            raise RuntimeError(
                "two-hand left contact diagnostic found no safe candidate: "
                f"{diagnostics}"
            )
        ...
        factory.bpy.context.scene[
            "attack_sword_twohand_left_contact_diagnostic_v21"
        ] = json.dumps(payload, sort_keys=True)
        print(
            "ATTACK_SWORD_TWOHAND_LEFT_CONTACT_DIAGNOSTIC_V21_SELECTED="
...
def _write_single_frame_sheet(
    ...
    if len(artifacts) != 1:
        raise RuntimeError("two-hand left contact diagnostic expected one frame")
...
def _write_manifest(
    ...
    payload["attack_sword_twohand_left_contact_diagnostic_v21"] = json.loads(
        str(
            factory.bpy.context.scene[
                "attack_sword_twohand_left_contact_diagnostic_v21"
            ]
