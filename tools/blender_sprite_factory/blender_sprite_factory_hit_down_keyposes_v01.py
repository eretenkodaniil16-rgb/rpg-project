...
from hit_down_keyposes_builder_v01 import create_hit_down_keypose_actions_v01
from hit_down_keyposes_profile_v01 import load_hit_down_keyposes_profile_v01


PROFILE_PATH = SCRIPT_DIR / "hit_down_keyposes_profile_v01.py"
...
def _find_frames(
    ...
    *,
    ...
    if tuple(item.frame_number for item in matches) != (1, 2, 3, 4):
        raise RuntimeError(f"hit down v01 missing frames: {animation_id}")
    return matches
...
def _assert_boundary_contract(artifact: factory.FrameArtifact) -> None:
    ...
    if forbidden:
        raise RuntimeError(
            f"hit down v01 {label} touches forbidden boundaries: {forbidden}"
        )
    if counts["left"] > MAX_ALLOWED_LEFT_EDGE_PIXELS:
        raise RuntimeError(
            f"hit down v01 {label} exceeds approved onehand left-edge budget: "
            f"{counts['left']}"
        )
...
def render_hit_down_keyposes_v01(
    ...
    try:
        ...
        if action is None or action.get("profile_revision") != profile.revision:
            raise RuntimeError("hit down v01 action is missing")
    ...
    if len(artifacts) != 4:
        raise RuntimeError(
            f"hit down v01 requires 4 rendered key poses, got {len(artifacts)}"
        )
    return artifacts
...
def _write_keypose_sheet(
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_hit_down_keyposes_v01",
        width=width,
        height=height,
        alpha=True,
        float_buffer=False,
...
def _write_manifest_v01(
    ...
    if not named_sheet.is_file():
        raise RuntimeError("hit down v01 contact sheet is missing")
    frames = _find_frames(artifacts, animation_id=profile.animation_id)
    ...
    payload["hit_down_keyposes_v01"] = {
        ...
        "locked_contract": {
            "guard_pose_exactly_preserves_approved_onehand_stance": True,
            ...
            "full_hit_cycle_not_yet_approved": True,
    ...
    payload.setdefault("animation_contract", {}).update(
        {
            "hit_01_current_stage": "down_keyposes_v01",
            "hit_01_keypose_count": len(frames),
            "hit_01_manual_review_required": True,
...
def main() -> int:
    base_adapter.create_combat_idle_down_actions_v01 = (
        create_hit_down_keypose_actions_v01
    )
    base_adapter.render_pilot_combat_idle_down_v01 = render_hit_down_keyposes_v01
