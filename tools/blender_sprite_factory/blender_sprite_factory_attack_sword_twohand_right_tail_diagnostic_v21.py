...
DIAGNOSTIC_SCENE_KEY = "attack_sword_twohand_right_tail_diagnostic_v21"
CONTACT_SHEET_NAME = "attack_sword_01_twohand_right_tail_diagnostic_v21.png"
...
def _projection_target_direction(
    ...
    *,
    ...
    if source_projection <= 1.0e-6:
        raise RuntimeError(
            "two-hand right tail diagnostic source projection is degenerate"
        )
...
def _diagnose_frame(
    ...
    *,
    ...
    minimum_clearance = float(
        MIN_HEAD_CLEARANCE_PIXELS_BY_FRAME[target_frame]
    )
    ...
    try:
                ...
                for offset_degrees in ANGLE_OFFSET_CANDIDATES:
                    ...
                    try:
                        ...
                        diagnostic: dict[str, object] = {
                            ...
                            "minimum_head_clearance_pixels": minimum_clearance,
                        ...
                        artifact, _ = export_adapter._render_candidate(
                            ...
                            output_name=(
                                ...
                                f"twohand_right_tail_diagnostic_v21_"
                        ...
                        diagnostics.append(diagnostic)
                        print(
                            "ATTACK_SWORD_TWOHAND_RIGHT_TAIL_"
        ...
        if selected is None or selected_artifact is None or selected_pose is None:
            raise RuntimeError(
                "two-hand right tail diagnostic found no safe candidate for "
        ...
        print(
            "ATTACK_SWORD_TWOHAND_RIGHT_TAIL_DIAGNOSTIC_V21_SELECTED="
            f"frame:{target_frame};"
            f"source:{source_frame};"
...
def _render_diagnostic(
    ...
    if action is None:
        raise RuntimeError(
            f"two-hand right tail action is missing: {TARGET_ACTION_ID}"
        )
    ...
    try:
        ...
        payload = {
            "revision": TWOHAND_RIGHT_TAIL_DIAGNOSTIC_REVISION,
...
def _write_five_frame_sheet(
    ...
    if [item.frame_number for item in ordered] != list(TARGET_FRAMES):
        raise RuntimeError(
            "two-hand right tail diagnostic expected frames f04-f08"
        )
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_attack_sword_twohand_right_tail_diagnostic_v21",
        width=width,
        height=tile_height,
        alpha=True,
        float_buffer=False,
...
def _write_manifest(
    ...
    payload["twohand_right_f02_f03_action_changed"] = True
...
def main() -> int:
    ...
    base_entry._write_contact_sheet_combat_idle_down_v01 = _write_five_frame_sheet
