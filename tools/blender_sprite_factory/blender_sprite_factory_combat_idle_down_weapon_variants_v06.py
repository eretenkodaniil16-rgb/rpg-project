...
import blender_sprite_factory_combat_idle_down_variants_v02 as calibration_adapter
import blender_sprite_factory_combat_idle_down_weapon_variants_v05 as rejected_adapter
...
from combat_idle_down_weapon_variants_builder_v06 import (
    ...
    create_weapon_stance_actions_v06,
)
...
PROFILE_PATH = SCRIPT_DIR / "combat_idle_down_weapon_variants_profile_v06.py"
BUILDER_PATH = SCRIPT_DIR / "combat_idle_down_weapon_variants_builder_v06.py"
COMPARISON_SHEET_NAME = "combat_idle_down_weapon_variants_v06.png"
...
def _require_objects(names: tuple[str, ...], label: str) -> None:
    ...
    if missing:
        raise RuntimeError(f"combat idle v06 is missing {label} objects: {missing}")


def _set_weapon_variant(variant_id: str | None) -> None:
    groups = {
        "baseline": COMBAT_WEAPON_OBJECT_NAMES,
        "onehand_v05": ONE_HAND_LONG_OBJECT_NAMES,
        "twohand_v05": TWO_HAND_LONG_OBJECT_NAMES,
        "onehand_low": ONE_HAND_V06_OBJECT_NAMES,
        "onehand_ready": ONE_HAND_V06_OBJECT_NAMES,
    ...
    for label, names in groups.items():
        ...
        enabled = variant_id == label or (
            variant_id in {"onehand_low", "onehand_ready"}
            and label in {"onehand_low", "onehand_ready"}
        )
        for name in names:
...
def render_weapon_stance_variants_v06(
    ...
    profile = load_weapon_stance_profile_v06(config.character_id)
    calibration = calibration_adapter._calibrate_idle_down(context, run_dir)
    raw_dir = run_dir / "raw"
    ...
    try:
        for variant in profile.variants:
            ...
            if action is None or action.get("profile_revision") != "v06":
                raise RuntimeError(
                    f"combat idle v06 cannot find action {variant.animation_id}"
                )
            ...
            _set_weapon_variant(variant.variant_id)
...
def _comparison_paths(
    ...
    animation_ids = (
        "combat_idle_onehand_low_v06",
        "combat_idle_onehand_ready_v06",
    ...
    for animation_id in animation_ids:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"combat idle v06 requires one {animation_id} frame, got {len(matches)}"
            )
...
def _write_variant_sheet(
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_combat_idle_down_weapon_variants_v06",
        width=width,
        height=height,
        alpha=True,
        float_buffer=False,
...
def _write_manifest_v06(
    ...
    if not comparison_path.is_file():
        raise RuntimeError("combat idle v06 comparison sheet is missing")
    ...
    for variant in profile.variants:
        ...
        if len(matches) != 1:
            raise RuntimeError(f"combat idle v06 artifact drifted: {variant.variant_id}")
        artifact = matches[0]
        rendered_variants.append(
            {
                ...
                "free_left_arm_sideways": (
                    variant.pose.upper_arm_left_z_degrees >= 26.0
                    if variant.grip_mode == "one_handed"
                    else None
                ),
                "rendered_frame": {
    ...
    payload["combat_idle_down_weapon_variants_v06"] = {
        ...
        "rejected_visual_revision": {
            "revision": "v05",
            "reason": "two_hand_blade_crossed_face_and_tip_did_not_clear_head",
        },
        ...
        "locked_contract": {
            ...
            "two_hand_blade_near_center_tip_up": True,
            "two_hand_blade_angled_away_from_face": True,
            "pointed_tips_used": True,
            "weapon_highlight_strips_used": True,
            "mirroring_used": False,
            "negative_scale_used": False,
        },
        "status": "technical_weapon_variant_set_requires_manual_selection",
    }
    payload.setdefault("animation_contract", {})
    payload["animation_contract"].update(
        {
            "combat_idle_down_active_stage": "weapon_variants_v06",
            "combat_idle_down_weapon_variant_count": len(profile.variants),
...
def main() -> int:
    base_adapter.create_combat_idle_down_actions_v01 = create_weapon_stance_actions_v06
    base_adapter.render_pilot_combat_idle_down_v01 = render_weapon_stance_variants_v06
    base_adapter._write_contact_sheet_combat_idle_down_v01 = _write_contact_sheet_v06
    base_adapter._write_run_manifest_combat_idle_down_v01 = _write_manifest_v06
