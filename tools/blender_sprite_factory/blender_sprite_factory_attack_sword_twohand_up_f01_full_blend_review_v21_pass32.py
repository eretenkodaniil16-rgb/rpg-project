...
DIAGNOSTIC_SCENE_KEY = (
    "attack_sword_twohand_up_f01_full_blend_review_v21_pass32"
)
CONTACT_SHEET_NAME = (
    "attack_sword_01_twohand_up_f01_full_blend_review_v21_pass32.png"
)
...
def _select_diverse_candidates(
    ...
    best_by_pose: dict[tuple[object, ...], dict[str, object]] = {}
    for candidate in sorted(candidates, key=_candidate_sort_key):
        ...
        best_by_pose.setdefault(pose_key, candidate)
    ordered = sorted(best_by_pose.values(), key=_candidate_sort_key)
    return tuple(ordered[:REVIEW_VARIANT_COUNT])
...
def _render_review(
    ...
    if up_action is None:
        raise RuntimeError(
            "two-hand up f01 full-blend review action is missing: "
            f"{TARGET_ACTION_ID}"
        )
    ...
    try:
        ...
        minimum_offset_by_blend: dict[str, float | None] = {}
        for arm_blend in ARM_BLEND_CANDIDATES:
            ...
            for source_order, source_frame in enumerate(
                ...
                blend_candidates.extend(accepted)
            minimum_offset_by_blend[f"{float(arm_blend):.2f}"] = (
                min(
                    abs(float(item["offset_degrees"]))
                    for item in blend_candidates
                )
                if blend_candidates
                else None
            )
            all_candidates.extend(blend_candidates)

        selected_candidates = _select_diverse_candidates(all_candidates)
        if not selected_candidates:
            raise RuntimeError(
                "two-hand up f01 full-blend review found no candidate within "
        ...
        for variant_index, candidate in enumerate(
            ...
            selection = f"full_blend_{variant_index:02d}"
            ...
            if REQUIRE_ZERO_EDGE_ALPHA and any(
                ...
                raise RuntimeError(
                    "two-hand up f01 full-blend review accepted edge pixels: "
                    f"{metadata}"
                )
            artifacts.append(artifact)
            columns.append(metadata)

        minimum_selected_offset = min(
            abs(float(item["offset_degrees"]))
            for item in selected_candidates
        )
        payload = {
            "correction_pass": CORRECTION_PASS,
            "revision": TWOHAND_UP_F01_FULL_BLEND_REVIEW_REVISION,
            ...
            "minimum_offset_by_blend": minimum_offset_by_blend,
            "minimum_selected_offset_degrees": minimum_selected_offset,
            "target_offset_reached": (
                minimum_selected_offset <= TARGET_ABS_WEAPON_OFFSET_DEGREES
            ),
        ...
        print(
            "ATTACK_SWORD_TWOHAND_UP_F01_FULL_BLEND_REVIEW_V21_PASS32="
            f"minimum_offset:{minimum_selected_offset:.1f};"
            f"target_reached:{minimum_selected_offset <= TARGET_ABS_WEAPON_OFFSET_DEGREES};"
...
def _write_review_sheet(
    ...
    if len(artifacts) != expected_count:
        raise RuntimeError(
            "two-hand up f01 full-blend review sheet count drifted: "
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_attack_sword_twohand_up_f01_full_blend_review_v21_pass32",
        width=width,
        height=tile_height,
        alpha=True,
        float_buffer=False,
