...
class CombatIdleDownVariantV02:
    variant_id: str
    display_name: str
    animation_id: str
...
@dataclass(frozen=True)
class CombatIdleDownVariantsProfileV02:
    character_id: str
    revision: str
    direction: str
    weapon_id: str
    weapon_hand: str
    variants: tuple[CombatIdleDownVariantV02, ...]

    def assert_valid(self) -> None:
        if self.character_id != "human_warrior_m01":
            raise ValueError("Combat idle variants belong to another character")
        if self.revision != "v02" or not re.fullmatch(r"v[0-9]{2}", self.revision):
            raise ValueError("Combat idle variants revision must be v02")
        if self.direction != "down":
            raise ValueError("The stance comparison must face down")
        if self.weapon_id != "sword_01" or self.weapon_hand != "right":
            raise ValueError("The approved warrior must use sword_01 in the right hand")
        if len(self.variants) != 3:
            raise ValueError("Combat idle v02 must contain exactly three variants")

        expected_ids = ("center_low", "center_mid", "diagonal_guard")
        actual_ids = tuple(item.variant_id for item in self.variants)
        if actual_ids != expected_ids:
            raise ValueError(f"Combat idle variant order drifted: {actual_ids}")
        if len({item.animation_id for item in self.variants}) != len(self.variants):
            raise ValueError("Combat idle variant animation IDs must be unique")
        ...
        for item in self.variants:
            ...
            if not -0.10 <= pose.pelvis_z <= -0.03:
                raise ValueError(f"Variant {item.variant_id} center of gravity is invalid")
            if pose.upper_arm_left_z_degrees <= baseline.upper_arm_left_z_degrees:
                raise ValueError(f"Variant {item.variant_id} must open the left arm farther")
            if pose.upper_arm_right_z_degrees <= baseline.upper_arm_right_z_degrees:
                raise ValueError(f"Variant {item.variant_id} must bring the sword arm inward")
            if abs(pose.upper_arm_left_x_degrees) > 18.0:
                raise ValueError(f"Variant {item.variant_id} overdrives the large left pauldron")
...
HUMAN_WARRIOR_M01_COMBAT_IDLE_DOWN_VARIANTS_V02 = CombatIdleDownVariantsProfileV02(
    ...
    variants=(
        CombatIdleDownVariantV02(
            ...
            animation_id="combat_idle_center_low",
            pose=_pose(
                phase="center_low",
                pelvis_x=0.0,
                pelvis_z=-0.060,
        ...
        CombatIdleDownVariantV02(
            ...
            animation_id="combat_idle_center_mid",
            pose=_pose(
                phase="center_mid",
                pelvis_x=-0.01,
                pelvis_z=-0.065,
        ...
        CombatIdleDownVariantV02(
            variant_id="diagonal_guard",
            display_name="Активная диагональная защита",
            animation_id="combat_idle_diagonal_guard",
            pose=_pose(
                phase="diagonal_guard",
                pelvis_x=0.01,
                pelvis_z=-0.070,
...
def load_combat_idle_down_variants_profile_v02(
    character_id: str,
) -> CombatIdleDownVariantsProfileV02:
    profile = HUMAN_WARRIOR_M01_COMBAT_IDLE_DOWN_VARIANTS_V02
    if character_id != profile.character_id:
        raise KeyError(
            f"No combat_idle_down variants v02 profile for character_id={character_id}"
        )
