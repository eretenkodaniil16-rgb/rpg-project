...
import blender_sprite_factory_attack_sword_directional_cycle_v21_pass15 as pass15_adapter
...
from attack_sword_directional_cycle_correction_v21_pass19 import (
    ...
    TAIL_DIAGNOSTIC_ARTIFACT_ID,
    TAIL_DIAGNOSTIC_ARTIFACT_SHA256,
    TAIL_DIAGNOSTIC_RUN_ID,
    TARGET_ACTION_ID,
    TARGET_BONES,
    TARGET_DIRECTION,
    TARGET_FRAMES,
...
CORRECTION_PATH = (
    SCRIPT_DIR / "attack_sword_directional_cycle_correction_v21_pass19.py"
)
BUILDER_PATH = (
    SCRIPT_DIR / "attack_sword_directional_cycle_builder_v21_pass19.py"
)
CONTACT_SHEET_NAME = "attack_sword_01_directional_cycle_v21.png"
BASE_RENDER_FRAME_PASS15 = pass15_adapter._render_frame_v21_pass15
BASE_WRITE_MANIFEST_PASS15 = pass15_adapter._write_manifest_v21_pass15
...
def _target_direction_v21_pass19(
    ...
    if source_projection <= 1.0e-6:
        raise RuntimeError(
            "attack sword directional v21 pass19 source projection is degenerate"
        )
...
def _render_frame_v21_pass19(
    ...
    *,
    ...
    if not _is_target_frame(animation_id, direction, frame_number):
        return BASE_RENDER_FRAME_PASS15(
            context,
    ...
    if action is None:
        raise RuntimeError(
            "attack sword directional v21 pass19 target action is missing"
        )
    if (
        action.get("directional_twohand_right_full_revision")
        != TWOHAND_RIGHT_FULL_REVISION
    ):
        raise RuntimeError(
            "attack sword directional v21 pass19 action metadata drifted"
        )
    ...
    target_direction, source_projection, applied_projection = (
        _target_direction_v21_pass19(current_direction, frame_number)
    )
    ...
    try:
        ...
        if clearance < minimum_clearance:
            raise RuntimeError(
                "attack sword directional v21 pass19 clearance drifted for "
                f"f{frame_number:02d}: {clearance:.3f}px"
            )
        if margin < MIN_CAMERA_MARGIN_PIXELS:
            raise RuntimeError(
                "attack sword directional v21 pass19 camera margin drifted for "
        ...
        if REQUIRE_ZERO_EDGE_ALPHA and touched:
            raise RuntimeError(
                "attack sword directional v21 pass19 touched export edges for "
    ...
    metrics[key] = {
        ...
        "pass19_arm_blend": SELECTED_ARM_BLEND_BY_FRAME[frame_number],
        "pass19_screen_projection": float(applied_projection),
    }
    ...
    print(
        "ATTACK_SWORD_DIRECTIONAL_V21_PASS19_SELECTED="
        f"{key};"
...
def _write_manifest_v21_pass19(
    context: factory.BuildContext,
    run_dir: Path,
    run_id: str,
    blend_path: Path,
    ...
    manifest_path = BASE_WRITE_MANIFEST_PASS15(
        context,
        run_dir,
        run_id,
        blend_path,
    ...
    if action is None:
        raise RuntimeError(
            "attack sword directional v21 pass19 manifest action is missing"
        )
    if (
        action.get("directional_twohand_right_full_revision")
        != TWOHAND_RIGHT_FULL_REVISION
        ...
        raise RuntimeError(
            "attack sword directional v21 pass19 manifest metadata drifted"
        )
    ...
    for frame_number in TARGET_FRAMES:
        ...
        if key not in metrics:
            raise RuntimeError(
                "attack sword directional v21 pass19 target metrics missing: "
                f"{key}"
            )
        target_metrics[f"f{frame_number:02d}"] = metrics[key]

    payload["attack_sword_directional_cycle_v21_pass19"] = {
        "correction_pass": CORRECTION_PASS,
        "twohand_right_full_revision": TWOHAND_RIGHT_FULL_REVISION,
        ...
        "windup_diagnostic_run_id": WINDUP_DIAGNOSTIC_RUN_ID,
        "windup_diagnostic_artifact_id": WINDUP_DIAGNOSTIC_ARTIFACT_ID,
        "windup_diagnostic_artifact_sha256": (
            WINDUP_DIAGNOSTIC_ARTIFACT_SHA256
        ),
        "anticipation_diagnostic_run_id": ANTICIPATION_DIAGNOSTIC_RUN_ID,
        "anticipation_diagnostic_artifact_id": (
            ANTICIPATION_DIAGNOSTIC_ARTIFACT_ID
        ),
        "anticipation_diagnostic_artifact_sha256": (
        ...
        "tail_diagnostic_run_id": TAIL_DIAGNOSTIC_RUN_ID,
        "tail_diagnostic_artifact_id": TAIL_DIAGNOSTIC_ARTIFACT_ID,
        "tail_diagnostic_artifact_sha256": TAIL_DIAGNOSTIC_ARTIFACT_SHA256,
        "target_render_metrics": target_metrics,
        "action_data_changed": True,
        "action_data_changed_frames": [2, 3],
        "rigid_weapon_transform_used": True,
        "approved_down_v20_changed": False,
        "left_direction_changed": False,
        "onehand_right_changed": False,
        "twohand_right_f01_changed": False,
        "up_actions_changed": False,
    ...
    payload.setdefault("animation_contract", {}).update(
        {
            "attack_sword_01_current_stage": (
                "directional_full_cycle_v21_pass19"
            ),
            "attack_sword_01_right_twohand_fixed": True,
...
def main() -> int:
    pass15_adapter._render_frame_v21_pass15 = _render_frame_v21_pass19
    pass15_adapter.create_attack_sword_directional_cycle_actions_v21_pass15 = (
        create_attack_sword_directional_cycle_actions_v21_pass19
    )
    pass15_adapter._write_manifest_v21_pass15 = _write_manifest_v21_pass19
    return pass15_adapter.main()
