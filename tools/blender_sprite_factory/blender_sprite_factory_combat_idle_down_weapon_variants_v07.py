...
import blender_sprite_factory_combat_idle_down_weapon_variants_v06 as previous_adapter
...
from combat_idle_down_weapon_variants_builder_v07 import (
    ...
    create_weapon_stance_actions_v07,
)
...
PROFILE_PATH = SCRIPT_DIR / "combat_idle_down_weapon_variants_profile_v07.py"
BUILDER_PATH = SCRIPT_DIR / "combat_idle_down_weapon_variants_builder_v07.py"
COMPARISON_SHEET_NAME = "combat_idle_down_weapon_variants_v07.png"
...
def _require_objects(names: tuple[str, ...], label: str) -> None:
    ...
    if missing:
        raise RuntimeError(f"combat idle v07 is missing {label} objects: {missing}")


def _set_weapon_variant_v07(variant_id: str | None) -> None:
    groups = {
        ...
        "onehand_low": ONE_HAND_LOW_V07_OBJECT_NAMES,
        "onehand_ready": ONE_HAND_READY_V07_OBJECT_NAMES,
    }
    for label, names in groups.items():
        _require_objects(names, label)
        enabled = variant_id == label
        for name in names:
...
def render_weapon_stance_variants_v07(
    ...
    profile = load_weapon_stance_profile_v07(config.character_id)
    calibration = previous_adapter.calibration_adapter._calibrate_idle_down(
        context,
        run_dir,
    )
    raw_dir = run_dir / "raw"
    ...
    try:
        for variant in profile.variants:
            ...
            expected_revision = "v07" if variant.grip_mode == "one_handed" else "v06"
            if action is None or action.get("profile_revision") != expected_revision:
                raise RuntimeError(
                    f"combat idle v07 cannot find {expected_revision} action "
            ...
            _set_weapon_variant_v07(variant.variant_id)
...
def _comparison_paths(
    ...
    for animation_id in animation_ids:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"combat idle v07 requires one {animation_id} frame, got {len(matches)}"
            )
...
def _write_variant_sheet(
    ...
    sheet = factory.bpy.data.images.new(
        "human_warrior_m01_combat_idle_down_weapon_variants_v07",
        width=width,
        height=height,
        alpha=True,
        float_buffer=False,
...
def _write_manifest_v07(
    ...
    profile = load_weapon_stance_profile_v07(context.config.character_id)
    comparison_path = run_dir / COMPARISON_SHEET_NAME
    if not comparison_path.is_file():
        raise RuntimeError("combat idle v07 comparison sheet is missing")
    ...
    for variant in profile.variants:
        ...
        if len(matches) != 1:
            raise RuntimeError(f"combat idle v07 artifact drifted: {variant.variant_id}")
        artifact = matches[0]
        rendered_variants.append(
            {
                ...
                "source_revision": (
                    "v07" if variant.grip_mode == "one_handed" else "v06"
                ),
                "rendered_frame": {
    ...
    payload["combat_idle_down_weapon_variants_v07"] = {
        ...
        "one_hand_correction": {
            ...
            "trajectory": "side_and_behind_hero",
        },
        ...
        "locked_contract": {
            ...
            "one_hand_blade_moves_side_and_behind": True,
    ...
    payload["animation_contract"].update(
        {
            "combat_idle_down_active_stage": "weapon_variants_v07",
            "combat_idle_down_one_hand_revision": "v07",
...
def main() -> int:
    base_adapter.create_combat_idle_down_actions_v01 = create_weapon_stance_actions_v07
    base_adapter.render_pilot_combat_idle_down_v01 = render_weapon_stance_variants_v07
    base_adapter._write_contact_sheet_combat_idle_down_v01 = _write_contact_sheet_v07
    base_adapter._write_run_manifest_combat_idle_down_v01 = _write_manifest_v07
