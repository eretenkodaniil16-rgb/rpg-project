...
const_marker = "const SQUAD_SEARCH_OFFSET_PIXELS: float = 132.0\n"
if "WORLD_INTERACTION_ACTION_ID" not in runtime:
    runtime = runtime.replace(
        const_marker,
        const_marker + "const WORLD_INTERACTION_ACTION_ID: String = \"world_interact\"\n",
        1,
    )

var_marker = "var _announced_plan_by_squad: Dictionary = {}\n"
if "_used_world_interaction_turn_token" not in runtime:
    runtime = runtime.replace(
        var_marker,
        var_marker + "var _used_world_interaction_turn_token: String = \"\"\n",
        1,
    )

methods_marker = "\n\nfunc _available_targets() -> Array[Node]:\n"
if methods_marker not in runtime:
    runtime += r'''
...
scene = scene.replace(
    "res://scripts/game/game_interactive_tactical_runtime.gd",
    "res://scripts/game/game_squad_tactical_plans_runtime.gd",
)
