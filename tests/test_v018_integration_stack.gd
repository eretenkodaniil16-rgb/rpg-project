...
const EXPECTED_GAME_RUNTIME: String = "res://scripts/game/game_pursuit_escape_runtime.gd"
...
func _run() -> void:
	...
	if game_script == null or game_script.resource_path != EXPECTED_GAME_RUNTIME:
		_fail("Game scene does not use the final pursuit/AI runtime.")
		return
	...
	await process_frame
	_cleanup_save()
	print("v0.18 integration stack, save/load, runtime and HUD contract passed.")
	quit(0)
