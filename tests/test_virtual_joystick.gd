...
const JOYSTICK_SCRIPT: Script = preload("res://scripts/ui/virtual_joystick.gd")
...
func _run_tests() -> void:
	...
	var full_right: Vector2 = JOYSTICK_SCRIPT.calculate_output_vector(Vector2.RIGHT, 0.16)
	assert(full_right.is_equal_approx(Vector2.RIGHT))

	var diagonal: Vector2 = JOYSTICK_SCRIPT.calculate_output_vector(Vector2(0.8, -0.8), 0.16)
	assert(diagonal.x > 0.0)
	assert(diagonal.y < 0.0)
	assert(diagonal.length() <= 1.001)

	var partial: Vector2 = JOYSTICK_SCRIPT.calculate_output_vector(Vector2(0.5, 0.0), 0.16)
