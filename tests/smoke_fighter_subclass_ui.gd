...
func _run() -> void:
	...
	if str(game.get_script().resource_path) != "res://scripts/game/game_fighter_subclasses_runtime.gd":
		_fail("Game scene does not use the fighter subclass runtime layer.")
		return
