...
Активный кандидат `proxy_v04` начинает отдельный проход `head v01`.
Он не меняет принятую геометрию тела, камеру, палитру, экипировку или Actions.
Голова получила отдельную суженную челюсть, открытую лицевую плоскость,
раздельные массы тёмных волнистых волос и минимальные геометрические
брови/глаза/рот. Профиль головы версионируется и хешируется отдельно от
силуэта, чтобы будущие лица не требовали копирования body profile.

## Следующие расширения после принятия пилота

1. проверить `head v01` по новому реальному рендеру `proxy_v04`;
