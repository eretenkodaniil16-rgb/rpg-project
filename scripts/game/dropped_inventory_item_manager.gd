...
func collect_drop(drop_id: String, save_after: bool = true) -> bool:
	...
	if definition.is_empty():
		return false
	var current: int = GameState.get_item_count(item_id)
	...
	if current + quantity > maxi(maximum, 1):
		...
		return false
	var updated: int = GameState.add_item(item_id, quantity, false)
...
func restore_from_world_snapshot() -> void:
	...
	_nodes.clear()
	var snapshot: Dictionary = GameState.get_world_snapshot()
...
func _normalize_record(fallback_drop_id: String, source: Dictionary) -> Dictionary:
	...
	if item_id.is_empty() or drop_id.is_empty() or GameState.get_item_definition(item_id).is_empty():
		return {}
