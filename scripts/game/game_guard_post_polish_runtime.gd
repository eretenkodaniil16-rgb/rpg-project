extends "res://scripts/game/game_controllable_ally_mobile_runtime.gd"
...
func resolve_npc_attack(
	...
	if _enemy_should_attack_controllable_ally(attacker):
		return _resolve_npc_attack_against_controllable_ally(
			attacker,
			attack_bonus,
			damage_die,
			damage_bonus,
...
func _resolve_npc_attack_against_controllable_ally(
	attacker: Node,
	attack_bonus: int,
	damage_die: int,
	damage_bonus: int,
	...
	if not is_instance_valid(attacker) or not attacker is Node2D:
		return {"hit": false, "status": "invalid_attacker"}
	if not is_instance_valid(_controllable_ally) or not _controllable_ally is Node2D:
		return {"hit": false, "status": "ally_missing"}
	...
	if bool(adjustments.get("blocked", false)):
		return {"hit": false, "blocked": true, "target": "ally"}
	var roll: Dictionary = {}
	if natural_override >= 1:
		var fixed_natural: int = clampi(natural_override, 1, 20)
		roll = {
			"natural": fixed_natural,
			"total": fixed_natural + attack_bonus
		}
	else:
		roll = _srd_rules.roll_d20(
			attack_bonus,
			bool(adjustments.get("advantage", false)),
			bool(adjustments.get("disadvantage", false)) or _controllable_ally_is_dodging()
		)
	var natural_roll: int = int(roll.get("natural", 1))
	var target_ac: int = _controllable_ally_armor_class() + int(cover.get("bonus", 0))
	...
	var damage_result: Dictionary = _apply_damage_to_controllable_ally(
		maxi(damage, 0),
		damage_type,
		critical,
		attacker
...
func _apply_damage_to_controllable_ally(
	amount: int,
	damage_type: String,
	...
	var source_name: String = "Источник"
	if is_instance_valid(source):
		source_name = _target_name(source)
	show_combat_message(
...
func _controllable_ally_armor_class() -> int:
	if is_instance_valid(_controllable_ally) and _controllable_ally.has_method("get_armor_class"):
		return int(_controllable_ally.call("get_armor_class"))
	return 10


func _controllable_ally_is_dodging() -> bool:
	if is_instance_valid(_controllable_ally) and _controllable_ally.has_method("is_dodging"):
		return bool(_controllable_ally.call("is_dodging"))
	return false
...
func apply_damage_to_controllable_ally_for_testing(
	...
	return _apply_damage_to_controllable_ally(
		amount,
		"slashing",
		critical_hit,
		null
