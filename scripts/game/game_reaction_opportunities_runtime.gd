...
func _request_coordinated_reaction(
	...
	return await _request_next_coordinated_reaction(title, details, session)


func _create_coordinated_reaction_session(
	trigger_id: String,
	base_context: Dictionary,
	source: Node = null,
	target: Node = null,
...
func _request_next_coordinated_reaction(
	title: String,
	details: String,
	session: Dictionary
) -> Dictionary:
	...
	if event == null or not _reaction_coordinator.should_continue(event):
		return {}
	_active_reaction_event = event
	var candidates: Array[ReactionCandidate] = []
	var candidate_values: Variant = session.get("candidates", [])
	if candidate_values is Array:
		for value: Variant in candidate_values as Array:
			if value is ReactionCandidate:
				candidates.append(value as ReactionCandidate)
