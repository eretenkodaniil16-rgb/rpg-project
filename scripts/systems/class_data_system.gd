...
func ensure_starting_loadout(character: PlayerCharacter) -> bool:
	...
	if items_value is Dictionary:
		for item_id_value: Variant in (items_value as Dictionary).keys():
			...
			GameState.add_item(item_id, int((items_value as Dictionary)[item_id_value]), false)
...
func get_equipped_weapon(character: PlayerCharacter) -> Dictionary:
	return GameState.get_item_definition(character.equipped_weapon_id) if not character.equipped_weapon_id.is_empty() else {}


func get_armor_class(character: PlayerCharacter) -> int:
	var dexterity: int = character.get_ability_modifier("dexterity")
	var armor: Dictionary = GameState.get_item_definition(character.equipped_armor_id)
	var shield: Dictionary = GameState.get_item_definition(character.equipped_shield_id)
	var armor_class: int
