...
func _install_cycle_navigation(
	previous_name: String,
	next_name: String,
	caption: String,
	previous_tooltip: String,
	next_tooltip: String,
...
func _rebuild_race_carousel() -> void:
	...
	for race_data: Dictionary in _races:
		entries.append({
			"id": str(race_data.get("id", "human")),
			"symbol": str(race_data.get("selection_symbol", "??")),
			"title": str(race_data.get("name", "Раса")),
			"subtitle": str(race_data.get("ability_bonus_description", "")),
			"accent": str(race_data.get("color_hex", PlayerCharacter.DEFAULT_APPEARANCE_COLOR_HEX))
		})
...
func _rebuild_class_carousel() -> void:
	...
	for class_data: Dictionary in _classes:
		...
		entries.append({
			"id": class_id,
			"symbol": str(class_ui.get("symbol", "??")),
			"title": str(class_data.get("name", "Класс")),
			"subtitle": str(class_data.get("role", "")),
			"accent": str(class_ui.get("accent", "#66778B"))
		})
...
func _rebuild_cyclic_carousel(carousel_name: String, entries: Array[Dictionary], selected_id: String, selector_kind: String) -> void:
	...
	for copy_index: int in range(CAROUSEL_COPY_COUNT):
		for entry: Dictionary in entries:
			...
			var card: Button = _make_selector_card(
				str(entry.get("symbol", "??")),
				str(entry.get("title", "Вариант")),
				str(entry.get("subtitle", "")),
				accent,
				entry_id == selected_id,
				_select_carousel_entry.bind(selector_kind, entry_id)
			)
...
func _configure_cyclic_carousel(carousel: ScrollContainer, ids: Array[String], selected_id: String, selector_kind: String) -> void:
	...
	carousel.scroll_horizontal = roundi(cycle_width + float(selected_index) * SELECTOR_STEP_WIDTH - centered_offset)
	var scroll_bar: HScrollBar = carousel.get_h_scroll_bar()
	if scroll_bar != null:
		scroll_bar.value_changed.connect(_on_cyclic_scroll_changed.bind(carousel, cycle_width))
	if carousel.has_signal("scroll_ended"):
		carousel.connect("scroll_ended", Callable(self, "_on_cyclic_carousel_scroll_ended").bind(carousel, ids, selector_kind))


func _on_cyclic_scroll_changed(value: float, carousel: ScrollContainer, cycle_width: float) -> void:
	if not is_instance_valid(carousel) or cycle_width <= 0.0:
		return
	var key: int = carousel.get_instance_id()
	if bool(_carousel_recentering.get(key, false)):
		return
	var scroll_bar: HScrollBar = carousel.get_h_scroll_bar()
	if scroll_bar == null:
		return
	var shifted_value: float = value
	var edge_zone: float = cycle_width * 0.35
	if value < edge_zone:
		shifted_value = value + cycle_width
	elif value > scroll_bar.max_value - edge_zone:
		shifted_value = value - cycle_width
	if is_equal_approx(shifted_value, value):
		return
	_carousel_recentering[key] = true
	carousel.scroll_horizontal = roundi(shifted_value)
...
func _on_cyclic_carousel_scroll_ended(carousel: ScrollContainer, ids: Array[String], selector_kind: String) -> void:
	if not is_instance_valid(carousel) or ids.is_empty():
		return
	...
	_select_carousel_entry(selector_kind, ids[logical_index])
